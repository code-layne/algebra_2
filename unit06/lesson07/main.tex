%! Inverse Functions — Unit 6, Lesson 6.7 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 6.7: Inverse Functions}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    6.0 by proving that $y=x^{2}$ has two arms and $y=x^{3}$ has one, from which every contrast
    ...
    Homework item~4). It rests directly on \textbf{A2.F.2k} (Lesson 6.6's composition, now used as
...
\vspace{-6pt}
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  \item ``Inverse'' is not new information --- students said it in their own words in 6.6 Tier A. Today it gets a name, a test, and a construction.
  \item \emph{Composition is the proof.} Yesterday's tool is today's evidence, which is exactly why 6.6 had to come first.
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    cube. \textbf{(2)} Lesson 6.6's Tier~A discovery, promoted to a warm-up: $f(x)=2x+6$ and
...
back at 6.0. Row (d) is where the lesson gets hard: $f^{-1}(x)=x^{2}+2$ \textbf{for $x\ge0$}, and
...
table students built in 6.0 --- then the two graphs with $y=x$ dashed. Read the linear picture
...
the horizontal line test on four pre-drawn graphs (line, parabola, square root, and a Unit~4 cubic
...
their own inverse: $6-x$, $\frac1x$, and $\frac{2x+3}{x-2}$ (a Unit~5 callback), whose graphs are
...
graph in 6.0), and part (f) is the quiet one: every restricted domain this week had to be argued
for, and here the situation supplies it for free.
\textbf{Preview:} the \textbf{Unit 6 test}. This lesson closes the unit --- rational exponents,
...
Lesson 6.0 opened: $x^{2}$ has two arms and $x^{3}$ has one, and every difference between $\sqrt{x}$
