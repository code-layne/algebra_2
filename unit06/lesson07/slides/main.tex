%! Inverse Functions — Unit 6, Lesson 6.7 — Slides (Beamer)
...
\begin{frame}[plain]
  ...
  \begin{minipage}{0.9\textwidth}
    {\color{goldacc}\bfseries\small LESSON 6.7}\\[0.5cm]
...
% THE CAPSTONE
\begin{frame}[t, plain]
  \forestheader{The question Lesson 6.0 opened --- answered}
...
% ROADMAP
\begin{frame}[t, plain]
  \forestheader{That is Unit 6 --- the loop is closed}
  \sectionlabel{Connections}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \footnotesize
      \begin{itemize}\setlength{\itemsep}{3pt}
        \item \textbf{6.0} --- why does $\sqrt{x}$ get only half the number line? \emph{(asked)}
        \item \textbf{6.1} --- radicals are exponents in disguise
        \item \textbf{6.2--6.3} --- simplify, operate, rationalize
        \item \textbf{6.4} --- graph and transform; the endpoint is the anchor
        \item \textbf{6.5} --- solve, and always check for extraneous roots
        \item \textbf{6.6} --- composition; order is part of the answer
        \item \textbf{6.7} --- inverses. \emph{(answered)}
      \end{itemize}
    \end{column}
    \begin{column}{0.44\textwidth}
      \footnotesize
      \begin{block}{Next: the Unit 6 test}
