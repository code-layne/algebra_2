%! Composition of Functions — Unit 6, Lesson 6.6 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 6.6: Composition of Functions}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    prerequisite for \textbf{A2.F.2i/j} in Lesson 6.7 --- composition is the \emph{verification} tool
    for inverses ($f(g(x))=g(f(x))=x$), and Tier~A rows (d)/(e) plus Tier~E Part~4 are built to plant
    that. It revisits \textbf{A2.F.1b/c} (6.4's transformations, now revealed as compositions),
    \textbf{A2.F.2a} (domain, computed through two gates), \textbf{A2.EO.2a} (6.1's
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \emph{A composition remembers where it has been.} This is the whole reason 6.7 must restrict the domain of $y=x^{2}$.
  \item Every transformation from 6.4 was a composition --- which is finally \emph{why} inside changes act backwards.
...
\textbf{5. Order matters, and the 6.4 payoff.} The anchor: $f(x)=\sqrt{x}$, $g(x)=x-4$. One order
gives $\sqrt{x-4}$, the other $\sqrt{x}-4$ --- two graphs students already read in Lesson 6.4, with
different endpoints \emph{and} different domains. Put both on the board and ask which one
$f(g(x))$ produces \emph{before} computing. Then deliver the payoff: every transformation from 6.4
...
$-9$, because Gate 1 turned it away before stage 2 ever ran. Beside it, $\sqrt{x^{2}}=|x|$ (6.1),
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item \textbf{$(x+1)^{2}=x^{2}+1$} --- the Unit 4 perfect-square error resurfacing inside a composition. It is exit-ticket distractor C;
  \item \textbf{reading the domain off the simplified rule.} The tell is answering ``all reals'' for $\left(\sqrt{x}\,\right)^{2}$ or for Tier A Part 3(c). This is the one that must be fixed \emph{before} 6.7;
  ...
  \item \textbf{$\sqrt{x^{2}}$ written as $x$}, dropping the absolute value. That is 6.1's $\sqrt[n]{a^{n}}$ rule, and today is the only place this week where variables are not assumed non-negative.
\end{itemize}
...
open Lesson 6.7. J2 is the assessed conceptual item: state both domains in row (c) and say
...
steps, then compared with the 6.4 transformation list --- \emph{a transformation is a composition}.
\emph{(4)} The $\left(\sqrt{x}\,\right)^{2}$ versus $\sqrt{x^{2}}$ table at $x=-4,-1,0,4,9$, ending
on the question that opens 6.7: what has to be done to $y=x^{2}$ first?
\end{multicols}
\end{skillbox}
...
\textbf{A} is the order error (it is $(g\circ f)$), \textbf{C} is the Unit 4 perfect-square error
...
argument in 6.7 will be arbitrary to them. Re-run Notes \S6 with that group before starting 6.7.
\end{skillbox}
...
unit --- for what $x$ is $(p\circ q)(x)=0$? --- which is the seed of 6.7's swap-and-solve; five
...
justification item (\textbf{the bridge to 6.7}) explaining why $\left(\sqrt{x}\,\right)^{2}=x$ has
domain $x\ge0$ while $\sqrt{x^{2}}=|x|$ accepts everything. \textbf{Extension:} the horizon model
$d(h)=\sqrt{1.5h}$ from Lesson 6.4 composed with a mast climb $h(t)=6t+6$, giving
...
in 6.4: a real navigation formula is the parent plus a shift and a stretch --- and now students know
\emph{why}, because the shift and the stretch are the two machines they composed.
\textbf{Preview:} Lesson~6.7, \emph{Inverse Functions} --- some of today's pairs gave back $x$ in
...
the thread first pulled in Lesson 6.0.
\end{skillbox}
\end{document}
