%! Solving Radical Equations \& Extraneous Solutions — Unit 6, Lesson 6.5 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 6.5: Solving Radical Equations \& Extraneous Solutions}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    revisits \textbf{A2.EO.2c} (6.1's rational exponents, in \S6), \textbf{A2.F.1c}/\textbf{A2.F.2a}
    (6.4's graphs, now read as the two sides of an equation), and \textbf{A2.EI.4c} (5.6's rational
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item This is Lesson 5.6's structure in new clothes: a non-reversible step produces candidates, and only the check promotes them.
...
\textbf{Principal root (6.1)} & An even root is never negative --- so an isolated even root equal to a negative number has no real solution. \\
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    radical equation appears. \textbf{(3)} Factoring $x^{2}-11x+18=0$ from Unit~3, giving $2$ and
...
Work $\sqrt{x+4}-1=3$ ($x=12$) and name what the equation is \emph{asking} --- 6.0's function
...
Lesson~5.6 parallel: multiplying by something that might be zero, squaring something that might be
negative --- both are non-reversible steps, both produce candidates.

\columnbreak

\textbf{3. The free shortcut.} From 6.1, an even root is the \emph{principal} root and is never
...
6.0. Work $2\sqrt[3]{x+1}-3=1$ so they still see an isolation step, and insist on the check anyway:
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item \textbf{restricting a cube root} --- writing ``no solution'' for $\sqrt[3]{x-4}=-2$. This is the 6.0 error resurfacing, and it is the exit ticket's MC distractor B;
...
$A^{3}=B^{3}\Rightarrow A=B$. \emph{(3)} 6.2's pendulum $T(L)=2\pi\sqrt{L/32}$ run \emph{backwards}
--- $T=2\pi$ gives $L=32$ ft and $T=\pi$ gives $L=8$ ft, both checkable against 6.2's own table ---
...
that is a Unit~6 spine failure rather than a slip in today's procedure. Collect and sort into ``got
...
are correct. The fourth pile needs the \S4 contrast re-run before Lesson 6.6.
\end{skillbox}
...
\textbf{Extension:} the skid-mark model $s(d)=\sqrt{24d}$ mph from Lesson~6.0's homework, run
...
\textbf{Preview:} Lesson~6.6, \emph{Composition of Functions} --- today one operation undid another;
next that idea gets a notation, $(f\circ g)(x)=f(g(x))$, and an order that matters. In 6.7 it
becomes the test for inverses, finally explaining 6.0's reflection over $y=x$.
\end{skillbox}
\end{document}
