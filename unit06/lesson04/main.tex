%! Graphing Radical Functions \& Transformations — Unit 6, Lesson 6.4 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 6.4: Graphing Radical Functions \& Transformations}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    \textbf{A2.F.1a} (parent graphs, 6.0), \textbf{A2.EO.2a} (the product property from 6.2, which is
    what makes $f(kx)$ collapse into $kf(x)$), and \textbf{A2.EO.2c} (rational exponents from 6.1, in
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item The product property (6.2) lets a constant walk out from under a radical. That single fact is why a radical has only \emph{one} size dial --- and why the trick dies the moment a term sits outside the radical.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    inside/outside sign rule students have owned since Unit~2, which today gets a new anchor to move;
    ...
    $\sqrt[3]{8x}$ from Lesson~6.2, which sets up the day's genuine surprise. Take the last blank of
...
\begin{skillbox}[Hook]{forestbg}
    ...
    \textbf{``In Lesson 6.0, finding the domain of $\sqrt{x-3}$ meant solving $x-3\ge0$. Look at the
...
6.0's origin symmetry ($\sqrt[3]{-8}=-2=-\sqrt[3]{8}$), and set it beside $-\sqrt{x}$ and
...
$y=x+1$, $f(9x)=9x+1$ but $9f(x)=9x+9$. Preview Lesson~6.5.
\end{multicols}
\end{skillbox}
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item writing a restricted domain for a \emph{cube} root because it has an $h$ (the 6.0 error, resurfacing in new clothes);
...
transformation, rewritten in a single line with \textbf{rational exponents} (6.1),
...
a procedural gap that resurfaces immediately in Lesson~6.5, where $\sqrt{2x-10}=4$ has to be handled
...
Lesson~6.5, \emph{Solving Radical Equations \& Extraneous Solutions} --- isolate, raise to the $n$th
power, and \emph{always check}, because squaring destroys sign information the same way Unit~5's
