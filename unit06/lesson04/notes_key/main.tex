%! Modeling with Exponential Functions: Interest, Half-Life, and e — Unit 6, Lesson 6.4 — Guided Notes (Answer Key)
...
\vocabans{Principal, $P$}{the starting amount --- the money deposited or borrowed before any interest; it is the $a$ of Lesson 6.1's $y=ab^{x}$}
...
\vspace{3pt}
\textbf{Will the tracer ever be completely gone?} \ans{No} --- and the reason is Lesson 6.0's, not
...
as an error to most students --- the same instinct that made them distrust $x=\frac23$ in Lesson 6.3.
...
pure Lesson 6.0 and should be answered from the asymptote, not from arithmetic.
...
The contrast with $2^{x}=-3$ is the distinction Lesson 6.3's exit ticket was built to test, and it is
the one Unit 7 depends on. Do not name logarithms. Leave the four-equation list posted --- Lesson 7.4
takes it down.
\end{teachernote}
