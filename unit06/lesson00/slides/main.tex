%! Characteristics of Exponential Functions — Unit 6, Lesson 6.0 — Slides (Beamer)
...
\begin{frame}[t, plain]
  ...
  \begin{block}{The trade}
    Unit 5 took away half the \textbf{domain}. Unit 6 gives it back --- and takes away half the
    \textbf{range} instead.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \hline
  Linear (Unit 1) & the difference & $2,\ 5,\ 8,\ 11,\ 14$ \quad (add $3$) \\
  Quadratic (Unit 2) & the \emph{second} difference & $1,\ 4,\ 9,\ 16,\ 25$ \quad ($+2$ each time) \\
  \textbf{Exponential (Unit 6)} & the \textbf{ratio} & $2,\ 6,\ 18,\ 54,\ 162$ \quad (times $3$) \\
  \hline
  \end{tabular}
  \end{center}
  \vspace{4pt}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.58\textwidth}
      ...
      \begin{block}{Secretly the same graph}
        ...
        {\scriptsize (Lesson 6.2 turns this into a rule.)}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[scale=0.24]
        ...
        \end{scope}
      \end{tikzpicture}\\[3pt]
      {\footnotesize \textbf{Unit 5:} solid \emph{endpoint} --- the minimum $0$ \textbf{is} reached.
      \\[2pt]
      \textbf{Unit 6:} dashed \emph{asymptote} --- $0$ is \textbf{never} reached.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{3pt}
    \item \textbf{6.1} Exponential growth \& decay --- building $y=ab^{x}$, and percent $\to$ factor.
    \item \textbf{6.2} Graphing \& transformations --- $f(x)+k$ is the one that \emph{moves the asymptote}.
    ...
    \item \textbf{6.5} Exponential regression --- today's fingerprint test, applied to real data.
  \end{itemize}
\end{frame}
