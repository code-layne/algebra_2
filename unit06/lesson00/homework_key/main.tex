%! Characteristics of Exponential Functions — Unit 6, Lesson 6.0 — Homework (Answer Key)
...
Unit 5 $\rightarrow$ Unit 6 hinge and is worth grading closely. A complete answer must say that
$\sqrt{0}=0$ is an actual output while no input makes $2^{x}=0$; ``one is a point and one is a line''
alone is not enough.

\textbf{Problem 4} previews Lesson 6.5, where the same three-way test is applied to real scatterplot
...
Part (a)'s ``why $0.75$ and not $0.25$'' is the direct set-up for Lesson 6.1's percent-to-factor
translation.
\end{teachernote}
