%! Characteristics of Exponential Functions — Unit 6, Lesson 6.0 — Guided Notes (Answer Key)
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Unit 5's square root graph gave up half the \emph{inputs} --- it simply stopped somewhere. These two graphs run all the way across, so the domain is back. What did they give up instead? \ansline{Half the \emph{outputs}. Both graphs stay entirely above the $x$-axis, so the range is only the positive numbers.}
...
\hline
Linear (Unit 1) & the \emph{difference} & $2,\ 5,\ 8,\ 11,\ 14$ \quad (add \ans{3} each time) \\
Quadratic (Unit 2) & the \emph{second} difference & $1,\ 4,\ 9,\ 16,\ 25$ \quad (differences $3,5,7,9$; those change by $+2$ each time) \\
\textbf{Exponential (Unit 6)} & the \textbf{ratio} & $2,\ 6,\ 18,\ 54,\ 162$ \quad (multiply by \ans{3} each time) \\
\hline
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=1.7em, itemsep=3pt]
  ...
  \item \textbf{Why these two are secretly the same graph.} Since
        ...
        \ans{$y$}-axis. Lesson 6.2 makes this a rule.
\end{enumerate}
\end{notesbox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
  ...
  \item \textbf{The trade.} Unit 5 took away half the \ans{domain} (no negative radicands). Unit 6
...
error wearing three costumes. The side-by-side picture in Section 4 (Unit 5's solid endpoint next to
Unit 6's dashed asymptote) is the single most important image of the lesson; do not rush it.

\textbf{Section 1 pacing.} Do not spend long on $b>0$ and $b\ne1$ --- they are guardrails, not
content, and the full algebraic story of fractional exponents is Lesson 5.1 material students
already own. The fingerprint table is what earns the time: it is the compare-and-contrast standard
(\textbf{A2.F.2b}, \textbf{A2.F.1e}) and it returns as the opening move of Lesson 6.5.

\textbf{Section 3, item 7} is a preview, not a requirement. Students only need to notice that
$\left(\frac12\right)^{x}=2^{-x}$; the reflection \emph{rule} is Lesson 6.2's job. If the class is
...
consecutive outputs is exactly what Lesson 6.1 will ask students to do from a \emph{table}, and it
is the skill that makes 6.5's regression work interpretable.
\end{teachernote}
