%! Graphing Exponential Functions and Transformations — Unit 6, Lesson 6.2 — Homework (Answer Key)
...
\begin{spiralbox}
\textbf{Coming next --- Lesson 6.3: Solving Exponential Equations with a Common Base.} In problem 3
...
be matched. That one-to-one move runs entirely on Unit 5's exponent machinery --- $8^{x}=4$,
...
will cost them in 6.5 when a regression returns a negative coefficient.
...
exactly when $a$ and $k$ have opposite signs. Rows with $k=0$ can never cross --- that is Lesson 6.0's
...
$2^{x}=4$ and Z solves $2^{x}=4$ as well. Point it out --- it is the exact equation Lesson 6.3 opens
...
worth insisting students argue it that way, because that reasoning is what Lesson 6.3 formalizes. If a
student asks how to solve $(0.9)^{t}=2.118$ exactly, tell them the tool does not exist yet and that
Unit 7 builds it. Do not name logarithms.
\end{teachernote}
