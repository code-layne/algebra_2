%! Solving Exponential Equations with a Common Base — Unit 6, Lesson 6.3 — Student Packet Cover Sheet
...
\begin{center}
  ...
  {\color{white}\textbf{\large Lesson 6.3 \quad Solving Exponential Equations with a Common Base}}
\end{center}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=5pt]
  \item \ldots{} solve an exponential equation by rewriting \textbf{both sides as powers of one base} and then setting the exponents equal --- including cases that need Unit 5's negative and fractional exponents.
...
base, set the exponents equal, and solve what is left. Step 1 is Unit 5 work --- $8=2^{3}$,
...
because of a fact you proved back in Lesson 6.0: an exponential graph has \textbf{no turning points},
so it never uses the same output twice --- which means $b^{m}=b^{n}$ can only happen when $m=n$. That
is also why the base has to satisfy $b>0$ and $b\neq1$, the restriction Lesson 6.1 handed you: $1^{5}$
...
6.4 walks up to and Unit 7 finally solves.
\end{remindbox}
