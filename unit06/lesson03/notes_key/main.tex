%! Solving Exponential Equations with a Common Base — Unit 6, Lesson 6.3 — Guided Notes (Answer Key)
...
\vocabans{Power of a power, $\left(b^{m}\right)^{n}=b^{mn}$}{the Unit 5 rule that turns $\left(2^{3}\right)^{x}$ into $2^{3x}$; the exponents \emph{multiply}, they do not add}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  ...
  \item Which fact about the \emph{shape} of an exponential graph (Lesson 6.0) guarantees the right-hand picture can never look like the left-hand one? \ansline{It has no turning points --- it is always increasing or always decreasing, so no output is ever repeated.}
\end{itemize}
...
that's how exponentials are.'' Push to \emph{no turning points} (Lesson 6.0) $\Rightarrow$ no output is
ever repeated $\Rightarrow$ $b^{m}=b^{n}$ forces $m=n$. Every later step in the lesson is licensed by
that sentence, and Unit 7 will re-use it verbatim. Watch for the $\pm$ reflex on item 2 of the practice
box --- it is the Unit 2 habit arriving one family too late.

\textbf{Section 1's $b=1$ line is the payoff for a rule students accepted on faith in 6.1.} They were
...
reach --- a method fact. Students who blur them will tell you in Unit 7 that logarithms ``fix'' negative
outputs. Say the difference out loud, and leave the three unsolved equations posted; Lesson 6.4 opens
by pointing at them.
\end{teachernote}
