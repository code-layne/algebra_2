%! Characteristics of Logarithmic Functions — Unit 8, Lesson 8.0 — Guided Notes (Answer Key)
...
\vocabans{Base $b$}{the number being raised to a power, written low after ``$\log$''; as in Unit~7 it must satisfy $b>0$ and $b\ne1$}
...
arriving. That missing edge is a \ans{vertical asymptote}, the Unit~5 idea returning in a brand-new
family.
\end{notesbox}
...
Unit~8. If time is short, assign the practice box as bell-work for Lesson~8.1 rather than cutting
Section~4.
...
base are Lesson~8.1. This is the same staging that let students read $\sqrt{x}$ in Lesson~6.0 before
meeting rational exponents in 6.1.

\textbf{Two errors to expect.} (1) Writing the domain of $\log_2(x-1)$ as $[1,\infty)$ --- the
Unit~6 square-bracket habit. Logs need \emph{strict} inequality, and the square bracket is the tell
