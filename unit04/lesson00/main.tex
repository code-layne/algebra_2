%! Characteristics of Rational Functions — Unit 4, Lesson 4.0 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 4.0: Characteristics of Rational Functions}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    using a graph or equation) --- the row the \S3 spine marks new for Unit~4, together with
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    Unit~3 factoring toolkit, reused as the domain-restriction engine; \textbf{(2)} evaluating
...
asymptotes, arms flattening). Preview Lesson~4.1, \emph{simplifying rational expressions}.
\end{multicols}
\end{skillbox}
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item reading restrictions off the \emph{simplified} expression, which erases the hole (the same mistake that produces extraneous solutions in 4.6);
...
hole-vs-asymptote slip'' to decide whether Lesson~4.1 opens with a re-teach --- the last pile matters
most, since 4.1 is built on exactly that distinction.
\end{skillbox}
...
domain $t\ge0$ comes from context, not algebra). \textbf{Preview:} Lesson~4.1, \emph{Simplifying
