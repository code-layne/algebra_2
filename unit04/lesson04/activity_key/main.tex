%! Zeros of Polynomials: the Rational Root Theorem — Unit 4, Lesson 4.4 — Group Activity (Answer Key)
...
\begin{enumerate}[leftmargin=1.7em, itemsep=9pt]
  ...
  \item \textbf{Same four moves, on your own.} $g(x) = x^3 + 2x^2 - x - 2$
    ...
    \vspace{2pt}
    In Lesson 4.2 you could have factored this one by \emph{grouping}. Show that route in one line:
...
\begin{enumerate}[label=(\alph*), leftmargin=1.8em, itemsep=6pt]
  ...
  \item Where did the other two go? Write your group's best guess --- Lesson 4.5 will tell you
        whether you were right. \ansline{They are imaginary: $\sqrt{-4} = 2i$, so the missing solutions are $x = \pm 2i$ --- a complex-conjugate pair, exactly as in Unit 3.}
\end{enumerate}
\end{tcolorbox}

\begin{teachernote}
\textbf{Tier placement.} Assign from the exit ticket of Lesson~4.3: the ``sign flip on $r$'' and
...
the theorem stop feeling arbitrary. Part 2 inverts it, which is the skill Lesson~4.5 opens with
...
telling them; groups that recall complex numbers from Unit~3 will get there themselves, and the ones
...
has been walking since Lesson~4.0.
\end{teachernote}
