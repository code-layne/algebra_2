...
\begin{frame}[t, plain]
  ...
  \end{block}
  \vspace{2pt}
  {\footnotesize \textbf{Why:} find a zero, divide it out (Factor Theorem, 4.3), and the quotient has
...
\begin{frame}[t, plain]
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \footnotesize
      The unit anchor, from Lesson 4.0:
...
\begin{frame}[t, plain]
  ...
  \end{itemize}
  \vspace{4pt}
  \begin{block}{Lesson 4.0 said the same thing with a picture}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Coming next}
        \textbf{4.6} Graphing polynomial functions \& modeling --- end behavior (4.0), zeros
        (4.2--4.4), multiplicity (4.0), and today's complete count become \emph{one} procedure for
