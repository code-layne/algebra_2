%! Graphing Quadratic Functions & Transformations — Unit 2, Lesson 2.1 — Guided Notes & Practice (Answer Key)
...
% PAGE LOCKSTEP with ../notes/main.tex: \vterm -> \vtermans, \blank -> \ans, and the answer argument of
...
\newcommand{\vtermans}[2]{%
  \par\noindent\begin{minipage}[t][\vrowheight][t]{\linewidth}%
    \vspace*{5pt}\textbf{\textcolor{forest}{#1:}}\quad\ans{#2}%
  \end{minipage}\par}
...
\pageheader{Unit 2, Lesson 2.1}{Guided Notes \& Practice --- Answer Key}
...
\vtermans{Parent function $y=x^2$}{the simplest quadratic; every other parabola is a transformation of it}
\vtermans{Transformation (shift, stretch, reflection)}{a change to the parent's rule that slides, widens or narrows, or flips its graph}
\vtermans{Vertex form $a(x-h)^2+k$}{hands you the vertex $(h,k)$ and the axis $x=h$ with no work --- flip the sign inside}
\vtermans{Standard form $ax^2+bx+c$}{hands you the $y$-intercept $(0,c)$; the axis is $x=-\frac{b}{2a}$, and that $x$ gives the vertex}
\vtermans{Intercept (factored) form $a(x-p)(x-q)$}{hands you the zeros $p$ and $q$; the axis sits at their midpoint}
\end{vocabbox}
...
\par\vspace{4pt}
Which costume tells you the \textbf{vertex} fastest? \ans{vertex form $(x-1)^2-4$} \quad Which one tells you the
\textbf{zeros}? \ans{intercept form $(x+1)(x-3)$}
\par\vspace{4pt}
...
$(x-2)^2$ moves the parent \labelbox{2.6cm}{right 2}\par}
\\
...
now?}{1.8cm}{Slides the parent \emph{up} $3$. Vertex $(0,3)$, axis $x=0$.} &
\pcell{2}{$y=(x-2)^2$ --- which way does it move, and how far? Why does the sign look
wrong?}{1.9cm}{Right $2$. The bracket asks what to subtract to get back to $0$, so it moves the \emph{opposite} way.} \\ \hline
\end{probgrid}
\\ \hline
...
Then: at $x=2$, which graph is \emph{higher}? \ans{$y=3x^2$}
\par\vspace{2pt}
...
\textbf{$x$} & $0$ & $1$ & $2$ & $3$ \\
\hline
\textbf{$y=x^2$} & \ans{$0$} & \ans{$1$} & \ans{$4$} & \ans{$9$} \\
\textbf{$y=3x^2$} & \ans{$0$} & \ans{$3$} & \ans{$12$} & \ans{$27$} \\
\end{tabularx}
\\
...
vertex a maximum or a minimum?}{1.9cm}{Opens down ($a<0$); \emph{wider} ($|a|<1$); the vertex $(0,0)$ is a \emph{maximum}.} &
\pcell{5}{$y=4x^2$ and $y=\tfrac14 x^2$ share a vertex. Which is narrower, and what in $a$ told
you?}{1.9cm}{$y=4x^2$ is narrower: $|4|>1$. $|\tfrac14|<1$, so that one is wider.} \\ \hline
\end{probgrid}
\\ \hline
...
\end{tikzpicture}\par\vspace{2pt}
The dashed line is the \labelbox{3.4cm}{axis of symmetry}\par}
\\
...
\begin{probgrid}{|Y|Y|}
\pcell{6}{$y=(x-1)^2-4$ --- name $a$, $h$, and $k$. Vertex? Axis? Opens which way?}{1.8cm}{$a=1$, $h=1$, $k=-4$. Vertex $(1,-4)$, axis $x=1$, opens up.} &
\pcell{7}{$y=-2(x+3)^2+5$ --- vertex, axis, direction, and narrower or wider than the
parent.}{1.9cm}{$h=-3$, $k=5$: vertex $(-3,5)$, axis $x=-3$. Opens down, narrower ($|-2|>1$).} \\ \hline
\end{probgrid}
\\
& \textbf{8.} Run it backwards: a parabola has vertex $(4,-1)$, opens up, and is the same width
as the parent. Its rule in vertex form is $y=$ \ans{$(x-4)^2-1$}, and its $y$-intercept is
\ans{$(0,15)$}.
\\ \hline
...
it really go, and why?}{1.9cm}{Left $4$: $x+4=x-(-4)$, so $h=-4$. Subtract inside, go right; add, go left.} &
\pcell{10}{Same bracket, two forms. In $y=(x+4)^2$ the shift is which way? In $y=(x+4)(x-2)$
the zeros are what? Why is that not the same question?}{1.9cm}{Shift: left $4$. Zeros: $-4$ and $2$. Same bracket, opposite jobs --- the \emph{form} decides.} \\ \hline
\end{probgrid}
\\ \hline
...
maximum or a minimum?}{1.8cm}{Vertex $(2,9)$, axis $x=2$. $a=-1$ opens it down, so $9$ is a \emph{maximum}.} &
\pcell{12}{\textbf{Standard form.} $y=-x^2+4x+5$: the $y$-intercept straight off, then the axis
from $x=-\frac{b}{2a}$ (work below).}{1.8cm}{$c=5$, so $(0,5)$. Then $x=-\frac{4}{2(-1)}=2$: the axis is $x=2$.} \\ \hline
\end{probgrid}
\\
& \begin{work}
...
midpoint.}{1.8cm}{Zeros $-1$ and $5$ (flip each factor's sign). Midpoint $\frac{-1+5}{2}=2$, so $x=2$.} &
\pcell{14}{\textbf{The crux.} Do your three axes agree, and why must they? A classmate read
the zeros as $1$ and $-5$: what went wrong?}{1.8cm}{All three give $x=2$: one curve, three costumes. $(x+1)$ is zero at $-1$, not $1$.} \\ \hline
\end{probgrid}
\\ \hline
\end{guidednotes}
