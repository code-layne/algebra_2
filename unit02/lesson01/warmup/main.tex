...
\vspace{2pt}
Did the two expressions give the \emph{same} answer? \blank{1.2cm} \quad So how many different
curves are hiding in those two lines? \blank{1.4cm}

\vspace{7pt}
\textbf{2. Moving a parent graph (Lesson 1.3).} The parent is $y=|x|$, the V with its corner at
the origin.
\begin{itemize}[leftmargin=1.6em, itemsep=3pt]
  \item $y=|x|+2$ moves the parent \blank{1.6cm} (up / down) by \blank{0.9cm}.
  \item $y=|x-4|$ moves the parent \blank{1.6cm} (left / right) by \blank{0.9cm}.
  \item $y=-|x|$ opens \blank{1.6cm} (up / down), so its corner is a \blank{2.4cm}
        (maximum / minimum).
\end{itemize}
...
\end{center}
\vspace{-2pt}
The second graph moved \blank{1.6cm} (left / right) by \blank{0.9cm}. \quad Does that surprise
you, given the minus sign? \blank{4.6cm}
\end{notesbox}
