%! Solving by Square Roots & Completing the Square — Unit 2, Lesson 2.3 — Slides (Beamer)
...
\begin{frame}[t, plain]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \vspace{0.5cm}
      \begin{block}{$x^2=-4$?}
        No \emph{real} solution --- nothing real squares to a negative. (Fixed in 2.4.)
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{3pt}
        \item Vertex $(1,-4)$ --- exactly 2.0--2.1.
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{block}{Choose the smartest tool}
        \begin{itemize}\setlength{\itemsep}{3pt}
          \item Factors nicely $\Rightarrow$ \textbf{factor} (2.2).
    ...
    \begin{column}{0.45\textwidth}
      \begin{block}{Next}
        \textbf{2.4 Complex Numbers} --- for when the square root hits a \emph{negative}:
        \[ x^2=-9, \quad \sqrt{-9}=? \]
        Meet $i=\sqrt{-1}$.
      \end{block}
    \end{column}
