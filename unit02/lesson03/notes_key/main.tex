%! Solving by Square Roots & Completing the Square — Unit 2, Lesson 2.3 — Guided Notes (Answer Key)
...
no real number squares to a negative. Lesson~2.4 builds the numbers that fix this. Today every radicand
stays $\ge0$.}}
\end{notesbox}
...
The \emph{same} move rewrites a quadratic in vertex form (Lesson~2.1) --- so it also finds the vertex.
\begin{center}
\begin{minipage}{0.56\linewidth}
\textbf{Rewrite the anchor} $x^2-2x-3$ in vertex form:
\[ x^2-2x-3=(x-\textcolor{keyred}{\mathbf{1}})^2-\textcolor{keyred}{\mathbf{4}}. \]
(Half of $-2$ is $-1$; add and subtract $1$.) Vertex $(\,\textcolor{keyred}{\mathbf{1}},\,\textcolor{keyred}{\mathbf{-4}}\,)$ --- matching 2.0--2.1.
...
factors nicely & factoring (2.2) \\ \hline
  ...
  \item $(x+1)^2=9$
    \begin{work}
      ...
      x &= -1\pm3 \\
        &= 2,\ -4
    \end{work}
  \item $x^2+4x-6=0$ (complete the square)
    \begin{work}
      x^2+4x &= 6 \\
      x^2+4x+4 &= 10 \\
      (x+2)^2 &= 10 \\
