%! The Quadratic Formula & the Discriminant — Unit 2, Lesson 2.5 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 2.5: The Quadratic Formula \& the Discriminant}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    the general equation $ax^2+bx+c=0$ (the Lesson~2.3 move, done once in general), then use it fluently:
    write the equation in standard form, read off $a,b,c$ (with signs), and simplify --- including
    \emph{complex} answers when the radicand is negative (Lesson~2.4). They isolate the \textbf{discriminant}
    ...
    \textbf{A2.F.2d}. Builds directly on completing the square (\textbf{A2.EI.2b}, Lesson~2.3) and the
    imaginary unit $i$ (\textbf{A2.EO.4}, Lesson~2.4).
\end{tcolorbox}
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item A negative radicand is no longer a dead end: Lesson~2.4's $i$ turns it into a complex-conjugate pair.
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    Lesson~2.4 ($\sqrt{-16}=4i$, $\sqrt{20}=2\sqrt5$), so formula outputs land in simplest radical or
...
\begin{skillbox}[Hook]{forestbg}
    ...
    $x^2-6x+7=0$ (won't factor --- needs completing the square), $x^2+2x+5=0$ (complex roots, from 2.4),
...
Lesson~2.3 move, but with letters. Land the result and box it:
...
so $x=\dfrac{2\pm\sqrt{16}}{2}=\dfrac{2\pm4}{2}=3,-1$ --- the same zeros as factoring $(x+1)(x-3)$ in 2.2
and reading the graph in 2.0. \emph{Same curve, same roots, universal method.}
...
completing-the-square capstone produced in 2.4, now from the formula. Reinforce: a negative discriminant
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item stopping at a negative radicand instead of converting with $i$ (Lesson~2.4);
...
to decide whether Lesson~2.6 opens with a re-teach.
\end{skillbox}
...
solved with the formula and a find-the-parameter discriminant problem. \textbf{Preview:} Lesson~2.6,
...
radicals from 2.4 that a negative discriminant will produce. Item~3 rehearses standard form and reading
...
board so students see the formula is not magic. The reject-negative-radicand-here habit from 2.3 flips today:
...
that sets the root type. Tier E \#3 closes the loop back to Lesson 2.4 --- a complex root really does
...
\textbf{D} (the ``no real solution'' habit from 2.3 that 2.4 replaced). If item~3 or the $i$-conversion is
widely missed, reopen Lesson~2.6 with a 60-second discriminant-and-$i$ recap before starting systems.
\end{teachernote}
...
(the same firework from Lesson~2.0); if a student applies the formula to $-16t^2+32t+48=0$ directly they get
$t=\frac{-32\pm64}{-32}$, and clearing to $t^2-2t-3=0$ factors to $(t-3)(t+1)$ --- a nice
formula-vs-factoring check. \#2 makes the discriminant a dial; \#3 closes back to Lesson~2.4.
\end{teachernote}
