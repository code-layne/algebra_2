%! Characteristics of Exponential Functions — Unit 7, Lesson 7.0 — Guided Notes
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Unit 6's square root graph gave up half the \emph{inputs} --- it simply stopped somewhere. These two graphs run all the way across, so the domain is back. What did they give up instead? \writeline
...
\hline
Linear (Unit 2) & the \emph{difference} & $2,\ 5,\ 8,\ 11,\ 14$ \quad (add \blank{0.9cm} each time) \\
Quadratic (Unit 3) & the \emph{second} difference & $1,\ 4,\ 9,\ 16,\ 25$ \quad (differences $3,5,7,9$; those change by $+2$ each time) \\
\textbf{Exponential (Unit 7)} & the \textbf{ratio} & $2,\ 6,\ 18,\ 54,\ 162$ \quad (multiply by \blank{0.9cm} each time) \\
\hline
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=1.7em, itemsep=3pt]
  ...
  \item \textbf{Why these two are secretly the same graph.} Since
        ...
        \blank{1.6cm}-axis. Lesson 7.2 makes this a rule.
\end{enumerate}
\end{notesbox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
  ...
  \item \textbf{The trade.} Unit 6 took away half the \blank{2.0cm} (no negative radicands). Unit 7
