%! Characteristics of Logarithmic Functions — Unit 7, Lesson 7.0 — Homework (Answer Key)
...
something moves.'' A vertical shift changes outputs only; the asymptote does not budge. Lesson~7.2
formalizes this ($f(x+k)$ is the only transformation that moves a vertical asymptote --- the exact
mirror of what $f(x)+k$ did to the horizontal asymptote in 6.2), so a student who reasons it out here
arrives at 7.2 with the idea already built.
...
parts (c)--(e) unaided, they have the inverse relationship. Part (e) is the Unit~5 callback and is
