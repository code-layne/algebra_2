%! Characteristics of Exponential Functions — Unit 7, Lesson 7.0 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 7.0: Characteristics of Exponential Functions}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    taught side by side: \emph{domain} (all reals, handed back after Unit~6 took half of it away),
    ...
    row the \S3 spine marks new for Unit~7 --- \textbf{growth vs.\ decay and the constant
    multiplicative rate} --- and \emph{revisits} the horizontal asymptote (\textbf{●} in Unit~5) in
    ...
    is assessed in the Activity Tier~A table. \emph{Note:} Unit~7 has no \texttt{A2.EO} or
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \textbf{Compare and contrast} growth with decay, and exponential with the families of Units 2--6.
\end{itemize}
&
\vspace{-6pt}
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Unit 6's endpoint \emph{was} on the graph, so it was an absolute minimum. Unit 7's floor is not on the graph, so it is not.
  \item Unit 6 took away half the \textbf{domain}; Unit 7 hands the domain back and takes half the \textbf{range}. The escalation reverses.
  \item ``End behavior'' has had two answers all year. Today one arm answers $\pm\infty$ and the other answers \emph{it flattens} --- the mirror of 6.0's ``the graph stops.''
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    Lesson~2.0; and \textbf{(3)} a doubling-rumor table starting at $3$ people, which plants both
...
\begin{skillbox}[Hook]{forestbg}
    ...
    Open by naming what has changed since yesterday's unit: \textbf{``Unit 6's square root graph gave
    ...
    Land the trade explicitly: \emph{Unit 6 took away half the domain; Unit 7 hands the domain back
...
linear keeps a constant \emph{difference} (Unit 2), quadratic a constant \emph{second} difference
(Unit 3), exponential a constant \textbf{ratio}. Insist on the word \emph{multiply}. Close with the
...
plainly that ``$-\infty$'' is the reflex answer from Unit 4 and it is wrong here.
...
section by rewriting $\left(\frac12\right)^{x}=\frac{1}{2^{x}}=2^{-x}$ --- a Unit 6 exponent
identity, not a new rule --- and note that the two parents are one graph reflected. Flag Lesson 7.2
...
Unit 6's $\sqrt{x}$ beside $2^{x}$: solid endpoint versus dashed asymptote. Add the Unit 5 contrast
...
the skill 7.1 and 7.5 both need. Finish with the \textbf{A2.F.2b} growth-vs-decay table, making the
...
exponential does none of it. Preview Lesson 7.1, \emph{exponential growth \& decay}.
\end{multicols}
\end{skillbox}
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item writing the range as $[0,\infty)$ --- treating the asymptote as an attained value; this is \emph{the} signature error of the lesson, and it is the Unit 6 endpoint habit carried over unexamined;
...
$x$-intercept'' to decide whether Lesson~7.1 opens with a re-teach. Also check that students pulled
$a=8$ off the $y$-intercept --- 7.1 asks them to \emph{build} $y=ab^{x}$ from scratch, and anyone who
...
quadratic, with the quadratic disguised as fast growth); and one justification item on why Unit 6's
endpoint produced an absolute minimum while Unit 7's asymptote does not --- the unit hinge, and the
...
contextual one. \textbf{Preview:} Lesson~7.1, \emph{Exponential Growth \& Decay} --- building
