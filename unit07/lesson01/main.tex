%! Exponential Growth and Decay — Unit 7, Lesson 7.1 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 7.1: Exponential Growth and Decay}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    from Lesson~7.0 whenever a range or a horizontal asymptote is checked as part of validating a
    ...
    for \textbf{A2.ST.2d/e} in Lesson~7.5: a regression is worthless if students cannot say what $a$
    and $b$ mean once technology hands them over. \emph{Note:} Unit~7 has no \texttt{A2.EO} or
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Building is reading run backwards. Lesson 7.0 pulled $a$ and $b$ out of a graph; today the same two numbers get pushed in.
...
\textbf{Constant ratio (from 7.0)} & Dividing consecutive outputs gives the same number every time --- and that number \emph{is} $b$. \\
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    ``$8\%$'' will write $b=0.08$. \textbf{(2)} A two-row recall check from Lesson~7.0 --- pull $a$,
...
\begin{skillbox}[Hook]{forestbg}
    ...
    Lesson~7.0's fingerprint test in words. Then ask the question the whole lesson hangs on:
...
of it remains'') is the illegal base $b=1$ and pays off Lesson~7.0's restriction with a reason
...
$40=5b^{3}$ leaves a cube root --- Unit 6 machinery, and say so. Case 2 uses the graph through
...
contrast is Lesson~7.0's asymptote, in context.
\end{multicols}
\end{skillbox}
...
exponential and must be caught with Lesson~7.0's fingerprint test; a pre-drawn decay graph with
...
the wall Lesson~7.4 ends on, arriving a few days early, and it should be left standing. The last
part reruns Lesson~7.0's believability question on Ashford's asymptote. \textbf{Preview:}
Lesson~7.2, \emph{Graphing Exponential Functions \& Transformations} --- where $f(x)+k$ turns out to
