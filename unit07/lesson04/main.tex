%! Modeling with Exponential Functions: Interest, Half-Life, and e — Unit 7, Lesson 7.4 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 7.4: Modeling --- Compound Interest, Half-Life, and $e$}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    description and then read back from a table or a graph. \emph{Note:} as documented in 7.3, the
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Nothing here is a new function family --- it is Lesson 7.1's $y=ab^{x}$ with a shorter ``one period.''
  ...
  \item A fractional exponent means a time between two periods. It is normal, and Lesson 6.1 already licensed it.
  \item ``It never reaches zero'' is a claim about the asymptote (7.0), and it is where the model stops describing reality.
  \item Knowing exactly which questions this unit cannot answer is what makes Unit 8 necessary rather than arbitrary.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    periods in five years. This is Lesson 7.1's percent-to-factor translation (\texttt{A2.F.2f}) with
    ...
    $\left(\frac12\right)^{1.5}$ (Unit 6 rational exponents), then the question that matters: how many
    ...
    they already computed. \textbf{(3)} Lesson 7.3, twice: solve $2^{t}=32$ by common base, then look
...
\begin{multicols}{2}
\textbf{1. Cutting the year into pieces.} Start from Lesson 7.1's $y=ab^{x}$ and change exactly one
...
Then the Lesson 7.0 question --- will it ever be gone? --- answered from the \textbf{asymptote} and the
...
$t=14$ (\$1979.93) and $t=15$ (\$2078.93), so \textbf{the answer exists}. Then run Step 1 of Lesson 7.3
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item $r$ entered as a percent rather than a decimal ($n=4$, $r=8$), which is Lesson 7.1's error one unit later;
  ...
  \item answering ``the drug is gone'' or ``the truck is worthless'' without the asymptote --- the Unit 6 endpoint habit that 7.0 was built to break;
...
\emph{setup} pile needs practice, not re-teaching. A large \emph{wall} pile changes how 7.5 opens and
must be fixed before Unit 8: five minutes on the Section 4 graph with the gold line visibly crossing.
\end{skillbox}
...
physically wrong. That last beat is deliberate set-up for 7.5's extrapolation discussion.
\textbf{Preview:} Lesson~7.5, \emph{Exponential Regression \& Choosing a Model} --- where nobody hands
you the rate, and Lesson 7.0's fingerprint test decides the family before technology fits anything.
\end{skillbox}
\end{document}
