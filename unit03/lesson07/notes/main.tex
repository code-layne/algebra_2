...
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt]
  \item A firework launches from the ground: $h(t)=-16t^2+64t+80$. Find its \emph{maximum height} and \emph{when it lands}. \\ Max: \blank{3.0cm} \qquad Lands: \blank{2.4cm}
  \item A dog run against a wall uses $60$ ft of fence on three sides. What width $x$ gives the \emph{maximum area}, and what is that area? \\ $x=\blank{1.6cm}$ \qquad max area $=\blank{2.4cm}$
  \item A shop's daily revenue is $R(x)=-10x^2+300x$ ($x$ in \$1 price cuts). What price change \emph{maximizes} revenue, and what is the maximum? \\ $x=\blank{1.6cm}$ \qquad max $R=\blank{2.4cm}$
\end{enumerate}
\end{practicebox}
