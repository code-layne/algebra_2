%! Modeling with Quadratics — Unit 3, Lesson 3.7 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 3.7: Modeling with Quadratics}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    \emph{how much}. The algebra is entirely from Lessons 3.1--3.5 (vertex by $x=-b/2a$; zeros by factoring or the
    ...
    3.1) and solving a single quadratic by factoring / the quadratic formula (\textbf{A2.EI.2b}, Lessons 3.2 \& 3.5).
\end{tcolorbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    standard form via $x=-b/2a$ then evaluating (Lesson 3.1) --- the engine behind every ``maximum/minimum''
    question; \textbf{(2)} \emph{solving a quadratic} $=0$ by factoring (Lessons 3.2/3.5) --- what ``when does it hit
...
\begin{skillbox}[Hook]{forestbg}
    ...
    3.1--3.5. Today you supply the model, then read the story off it.
\end{skillbox}
...
(iv) Find it (Lessons 3.1--3.5). (v) \emph{Interpret with units}, rejecting values outside the feasible domain.
\end{multicols}
\end{skillbox}
...
ties back to systems (Lesson 3.6); verify and interpret.
\end{multicols}
\end{skillbox}
...
optimization and a same-height comparison (systems tie-back). \textbf{Preview:} this closes Unit 3 --- next is the
\emph{Unit 3 Review \& Test}, pulling together graphing, factoring, the quadratic formula, complex numbers, systems,
