%! Modeling with Quadratics — Unit 3, Lesson 3.7 — Guided Notes (Answer Key)
...
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt]
  \item A firework launches from the ground: $h(t)=-16t^2+64t+80$. Find its \emph{maximum height} and \emph{when it lands}. \\ Max: \ans{$144$ ft at $t=2$ s} \qquad Lands: \ans{$t=5$ s}
  \item A dog run against a wall uses $60$ ft of fence on three sides. What width $x$ gives the \emph{maximum area}, and what is that area? \\ $x=\ans{$15$ ft}$ \qquad max area $=\ans{$450$ ft$^2$ ($15\times30$)}$
  \item A shop's daily revenue is $R(x)=-10x^2+300x$ ($x$ in \$1 price cuts). What price change \emph{maximizes} revenue, and what is the maximum? \\ $x=\ans{$15$}$ \qquad max $R=\ans{$\$2250$}$
\end{enumerate}
\end{practicebox}
