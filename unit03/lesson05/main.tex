%! Fundamental Theorem of Algebra and Complex Zeros — Unit 3, Lesson 3.5 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 3.5: Fundamental Theorem of Algebra and Complex Zeros}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    determine the \emph{number and type} of solutions from a degree and a graph, finish Lesson~3.4's
    ...
    check. Built on \textbf{A2.EO.4a/b/c} (the imaginary unit and $a+bi$ arithmetic, Lesson~2.4),
    \textbf{A2.EI.2b} and the discriminant (Lesson~2.5), \textbf{A2.EO.3b/c} (factoring and division,
    Lessons~3.2--3.3), and \textbf{A2.F.2c} (multiplicity and $x$-intercepts, Lesson~3.0).
\end{tcolorbox}
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \textbf{Solve over $\mathbb{C}$}: run Lesson~3.4's workflow, then finish the depressed quadratic with square roots or the quadratic formula even when the discriminant is negative.
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Therefore an \emph{odd}-degree polynomial with real coefficients must have at least one real zero --- exactly what Lesson~3.0's opposite end-behavior arms forced graphically.
  ...
  \item Zeros $\rightarrow$ factors $\rightarrow$ polynomial is the same road as Lesson~3.4, driven in reverse.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\textbf{Multiplicity} & How many times a factor repeats; a double zero uses up \emph{two} of the $n$ slots (Lesson 3.0). \\
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    yesterday's cliffhanger. \textbf{(1)} \emph{Complex arithmetic from Lesson~2.4}: $i^2=-1$ and the
    conjugate product $(x-3i)(x+3i)=x^2+9$ --- with the observation that the $i$'s vanish and the
    result is \emph{real}. \textbf{(2)} \emph{Solving from Lessons~2.3 and 2.5}: $x^2+4=0$ by square
    roots ($x=\pm 2i$) and $x^2-2x+5=0$ by the quadratic formula ($D=-16$, $x=1\pm 2i$) --- both
    answers written down as a \emph{pair}. \textbf{(3)} \emph{Finishing Lesson~3.4's Tier E problem}:
...
\begin{skillbox}[Hook]{forestbg}
    ...
    Lesson~2.3, the negative discriminant in 2.5, yesterday's stalled depressed polynomial --- as the
...
graph students read in Lesson~3.0. Two \emph{distinct} zeros, three zeros \emph{counted with
...
degree forces at least one real zero. Tie that to Lesson~3.0: an odd-degree graph has arms pointing
...
divide, finish (Lesson~3.4). The change is the last step. When the depressed quadratic has a negative
...
\emph{not} zero, and $x^4-5x^2-36=0$ by quadratic form (Lesson~3.2) for $\pm3, \pm2i$.
...
a real zero, cross-checked against Lesson~3.0's end behavior. Then $x^4+13x^2+36=0$, whose four zeros
...
--- factor by grouping (Lesson~3.2) to $(x+1)(x^2+4)$, report $-1$, $2i$, $-2i$, and then state the
...
wrong.'' The conjugate pile is the one to fix on the spot, because Lesson~3.6 asks students to move
...
of escalating difficulty --- $x^3+2x^2+4x+8=0$ (the polynomial Lesson~3.3's homework produced as a
...
\textbf{Preview:} Lesson~3.6, graphing polynomial functions and modeling --- everything the unit has
