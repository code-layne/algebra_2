%! The Quadratic Formula & the Discriminant — Unit 3, Lesson 3.5 — Guided Notes (Answer Key)
...
\vspace{2pt}
\vocabans{Quadratic formula}{$x=\dfrac{-b\pm\sqrt{b^2-4ac}}{2a}$; solves any $ax^2+bx+c=0$ ($a\ne0$)}
\vocabans{Standard form $ax^2+bx+c=0$}{all terms on one side set equal to $0$; needed to read $a,b,c$}
\vocabans{Discriminant $b^2-4ac$}{the expression under the radical; its sign predicts the roots}
\vocabans{Double (repeated) root}{one real solution counted twice, when $b^2-4ac=0$ (vertex on the $x$-axis)}
\vocabans{Complex-conjugate roots}{the pair $a\pm bi$ produced when $b^2-4ac<0$}
\end{vocabbox}
...
Complete the square on the \emph{general} quadratic $ax^2+bx+c=0$ (same steps as Lesson 3.3, now with
letters). The result is the \textbf{quadratic formula}:
\[ x=\frac{-b\pm\sqrt{\rule{0pt}{1.6ex}\ans{$b^2-4ac$}}}{\ans{$2a$}}. \]
...
\end{enumerate}
\textbf{Anchor example.} Solve $x^2-2x-3=0$: \ $a=\ans{$1$}$, $b=\ans{$-2$}$, $c=\ans{$-3$}$.
\[ x=\frac{-(-2)\pm\sqrt{(-2)^2-4(1)(-3)}}{2(1)}=\frac{2\pm\sqrt{\ans{$16$}}}{2}=\frac{2\pm\ans{$4$}}{2}=\ans{$3,\ -1$}. \]
Same zeros as factoring $(x+1)(x-3)$ in 3.2 --- \emph{one method now solves it all.}
\end{notesbox}
...
\hline
$>0$ & \ans{two distinct real} & \ans{crosses the $x$-axis twice (two intercepts)} \\
$=0$ & \ans{one repeated real (double)} & \ans{vertex on the $x$-axis (one intercept)} \\
$<0$ & \ans{two complex conjugates} & \ans{never touches the $x$-axis (no intercepts)} \\
\end{tabularx}
\end{center}
\vspace{2pt}
\begin{center}
...
\[ x^2-6x+7=0:\ b^2-4ac=\ans{$8$}\ \Rightarrow\ \ans{two real (irrational)}. \]
\[ 4x^2-12x+9=0:\ b^2-4ac=\ans{$0$}\ \Rightarrow\ \ans{one repeated real root}. \]
\end{notesbox}

\begin{notesbox}{3. When the discriminant is negative --- complex solutions}
A negative discriminant is \emph{not} a dead end. Use $i$ (Lesson 3.4): the roots are a
\textbf{complex-conjugate pair} $a\pm bi$. Solve the 3.4 leftover $x^2+2x+5=0$:
\[ a=1,\ b=2,\ c=5:\quad b^2-4ac=4-20=\ans{$-16$}. \]
\[ x=\frac{-2\pm\sqrt{-16}}{2}=\frac{-2\pm\ans{$4i$}}{2}=\ans{$-1\pm2i$}. \]
These are the \emph{same} solutions completing the square gave in Lesson 3.4 --- now from the formula.
...
\[ -16t^2+24t+6=0 \ \Rightarrow\ t=\ans{$\dfrac{3\pm\sqrt{15}}{4}$} \ \Rightarrow\ t\approx\ans{$1.72$}\text{ s}. \]
\end{notesbox}

\begin{practicebox}
\textbf{Solve with the quadratic formula. Simplify; use $a+bi$ when needed.}
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt]
  \item $x^2+2x-5=0 \Rightarrow x=\ans{$-1\pm\sqrt{6}$}$
  \item $2x^2-4x+3=0 \Rightarrow x=\ans{$1\pm\dfrac{\sqrt{2}}{2}i$}$
  \item Discriminant only: how many/what type of roots does $9x^2-6x+1=0$ have? \ans{$b^2-4ac=0$: one repeated real root}
\end{enumerate}
\end{practicebox}
