...
% Deck follows the lesson's gradual-release flow (stats/SAAR two-section shape, 2026-09-07):
% targets → warm-up → hook (left unresolved) → I do (notes 1-2, four frames) → we do (guided
% practice) → debrief → close & start the homework. There is no group activity and no
...
% NOTES 1b — THE V IS TWO PIECES
\begin{frame}[t, plain]
  \forestheader{Notes 1, continued: the V is two pieces}
  ...
  \begin{block}{Meets at the vertex}
    ...
    \emph{meet}: one connected V. That is the first ``meets''; the next slide shows a ``jumps.''
  \end{block}
\end{frame}

% NOTES 2a — MEETS OR JUMPS
\begin{frame}[t, plain]
  \forestheader{Notes 2: open or closed, meets or jumps}
  \sectionlabel{Same story, two functions}
  \begin{columns}[T]
...
% NOTES 2b — THE CRUX, then rounding down
\begin{frame}[t, plain]
  \forestheader{Notes 2, continued: exactly one piece owns the boundary}
  \sectionlabel[redacc]{The crux --- the story cannot decide it; the inequality does}
  \begin{itemize}\setlength{\itemsep}{5pt}
  ...
  \begin{block}{Step functions: rounding down}
    ...
    $[n,n+1)$: closed left, open right --- the boundary rule, one step at a time.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel[goldacc]{You hold the pen --- one function, then change one piece}
  $p(x)=-2x$ for $x\le-1$, \; $x+3$ for $x>-1$ --- the graph is on your page.
  \begin{enumerate}\setlength{\itemsep}{4pt}
    \item[\textbf{a.}] Evaluate at $-3$, $0$, $2$ --- then at $-1$: which piece owns it, and how
          do you know?
    \item[\textbf{b.}] Meets or jumps at $-1$? \quad \textbf{c.} Decreasing where, increasing
          where, and the minimum?
    \item[\textbf{d.}] A classmate says ``both rules give $2$, so it does not matter who owns
          $-1$.'' Change the second piece to $x+4$ and test that.
    \item[\textbf{e.}] Round down: $\lfloor2.9\rfloor$, $\lfloor-1.2\rfloor$, $\lfloor4\rfloor$
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{5pt}
    ...
    \item \textbf{Part (d).} The first piece still owns $-1$ --- $p(-1)=2$, closed dot at
