...
% Deck follows the lesson's gradual-release flow, 11 frames (regenerated 2026-09-07): title →
% targets → warm-up → hook → notes 1–4 (I do / we do) → individual-practice launch (you do alone)
% → debrief → close & start the homework. No group activity. Homework is due the first class after
% two study halls (course policy, 2026-09-03).
...
\begin{frame}[t, plain]
  ...
  \begin{block}{Careful --- exactly one piece owns the boundary}
    ...
    $0\le m\le3$, so $P(3)=0$ --- the closed dot says so. The story cannot decide it; the
    inequality does.
  \end{block}
\end{frame}

% NOTES 4 — STEP FUNCTIONS, then the Guided Practice
\begin{frame}[t, plain]
  \forestheader{Notes 4: step functions --- the greatest-integer function}
  \sectionlabel{Then one together}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{6pt}
    ...
    \item \textbf{Two pieces and the staircase.} $|x-3|$ as two pieces; round down; then a
          classmate says the step $[4,5)$ includes $x=5$ --- what did they miss?
  \end{enumerate}
  \vspace{5pt}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{5pt}
    ...
    \item \textbf{3 --- two pieces, rounding down, the crux.} $|x-3|$ is $x-3$ on the right,
          $3-x$ on the left. $\lfloor-1.2\rfloor=-2$: down means more negative. And $5$ belongs to
          the \emph{next} step: $\lfloor5\rfloor=5$, closed dot at $(5,5)$ --- one step owns it.
  \end{enumerate}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \sectionlabel{10 minutes $\cdot$ the Homework page in your packet}
  ...
  \begin{block}{Homework --- scored, due the first class after two study halls}
    We start item~1 together right now; then you keep going, alone. Six problems plus an
    extension: evaluating with a boundary, a meets-or-jumps pair, reading a graph, the staircase,
    a phone-plan model, and one multiple-choice item. Whatever is left is due the first class
    after two study halls --- I will announce the date in class and on TurtleNet.
  \end{block}
  \vspace{4pt}
