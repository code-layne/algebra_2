...
% the Main Ideas / Notes table shape (2026-09-12; the notes are one two-column table, four
% instruction rows then a Guided Practice row). THERE IS NO GROUP ACTIVITY — the guided notes
% carry the release row by row: I do (the definition, the display, the first problem of each
% grid) -> we do (the rest of each grid, then the Guided Practice row). THERE IS NO
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    deliver the instruction in \textbf{one Main Ideas / Notes table} on one context (a streaming
    service, free for three months): four instruction rows --- \emph{Several Rules}, \emph{Two
    Pieces}, \emph{Meets or Jumps}, \emph{Boundary} --- each a definition, a display, and a grid
    of problems (1--9) where you work the first and the class works the rest; then the Guided
    Practice row (10--13) is worked together with students holding the pen;
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item \textbf{Target misconception:} that a boundary input belongs to ``both'' pieces, or to
        ...
        inequality includes it --- and the closed dot shows which. Notes problem~7 ($P(3)=0$, the
        crux) and Guided Practice problem~12 both test it.
...
Guided Notes \& Practice & 35 & Fill the vocabulary box and the notes table row by row as each
  term is named, working the rest of each grid themselves; then Guided Practice (10--13) together,
  pen in their hand & \textbf{I do} the hook, then each row's definition, display, and first
  problem (1, 3, 5, 7) (20) $\to$ \textbf{we do} the rest of each grid, then the Guided Practice
  row (15): $p(x)$, then change one piece and watch the boundary \\
...
\begin{minipage}[t]{0.48\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{2.} The V as two lines, $y=x$ for $x\ge0$ and $y=-x$ for $x<0$, with the
              check $|-6|=-(-6)$. \textbf{Seeds:} the whole bridge from Lesson~1.3 --- the
              \emph{Two Pieces} row writes it with a brace.
        \item \textbf{3.} Open vs.\ closed dots, $<$ vs.\ $\le$, ending on \emph{if one rule is
              ...
              boundary rule, which the \emph{Several Rules} row states and problem~7 tests.
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\linewidth}
...
hear one reason for each, and \emph{do not settle it}. Problem~2 settles it, and problem~7 makes
the wrong vote cost something. Fill the vocabulary rows \emph{as you go}, never in advance.
...
\emph{Several Rules (pages 1--2, 5 min).} Read the brace for $C(m)$ --- each rule comes with
the inputs it is \emph{for} --- and read the graph; the labelled feature is the \emph{boundary}.
The worked line is $C(5)=24$. State the \textbf{boundary rule} aloud: \emph{the story does not
decide it; the inequality does.} Work \textbf{1} ($C(2)=0$); the class works \textbf{2} --- $3$
satisfies $0\le m\le3$, so $C(3)=0$ and month~3 is free: the vote is settled.

\emph{Two Pieces (pages 2--3, 4 min).} $|x|$ with a brace, the check $|-3|$, the vertex label
$(3,1)$, then the trail $H(x)=2|x-3|+1$ in the two side-by-side \texttt{work} blocks ($2x-5$ /
$7-2x$). Work \textbf{3}: both rules give $H(3)=1$, so the pieces \emph{meet} --- the first
``meets'' example, and the \emph{Meets or Jumps} row needs it. The class works \textbf{4}.

\emph{Meets or Jumps (page 3, 3 min).} Total paid $C(m)$ beside price-this-month $P(m)$ on the
\emph{same} story. Land the one test: \emph{both rules agree at the boundary, or they do not.}
Work \textbf{5} ($C$: both give $0$, meets; constant on $[0,3]$, increasing after); the class
works \textbf{6} ($P$: $0$ and $12$, jumps; range $\{0,12\}$).

\emph{Boundary (pages 3--4, 5 min) is the lesson.} Label the two dots at $m=3$ (closed, open),
then the staircase (a step function): $\lfloor x\rfloor$ rounds down, $\lfloor-2.5\rfloor=-3$
(down means more negative), constant $n$ on $[n,n+1)$ --- the boundary rule one step at a time.
\textbf{7 is the crux:} let someone say ``$P(3)=12$, the price is \$12 after the free months''
and let the room catch it --- $3$ satisfies $0\le m\le3$, so $P(3)=0$, and the closed dot at
$(3,0)$ says so; the open dot at $(3,12)$ only marks where the second piece starts. Point at the
two dots while you say it. The class works \textbf{8} (in their own words) and \textbf{9} ---
the round-down table, whose last cell $\lfloor-2.5\rfloor$ is the trap, and ``does $[2,3)$ own
$3$?''

\columnbreak
\textbf{We do (about 15 min, page 4).} The Guided Practice row: $p(x)=-2x$ for $x\le-1$, $x+3$
for $x>-1$, with its graph pre-drawn. Students hold the pen; you hold the questions.
\textbf{10:} evaluate at $-3$, $0$, $2$, then $-1$ --- \emph{which piece owns it, and how do you
know?} \textbf{11:} meets or jumps at $-1$ (both give $2$: meets); decreasing on
$(-\infty,-1]$, increasing on $[-1,\infty)$, minimum $2$, read off the graph. \textbf{12 is the
crux transferred and the problem to protect:} change the second piece to $x+4$. The first piece
still owns $-1$ ($p(-1)=2$), the second now gives $3$ just past it, the graph jumps, and the
closed dot is at $(-1,2)$; the classmate who said ``both rules gave $2$, so it does not matter''
missed that \emph{ownership is decided by the inequality, not by the rules happening to agree.}
\textbf{13:} round down --- $\lfloor2.9\rfloor$, $\lfloor-1.2\rfloor$ (the trap: $-2$),
...
can watch a student decide, so do not narrate it. Put problem~10's $p(-1)$ on them cold, take a
hand count on \emph{meets or jumps} in problem~12 before anyone speaks, and make the room defend
both answers.

Circulate during Guided Practice and demand the reason, not the word:
\begin{itemize}[leftmargin=1.1em, nosep]
  \item \textbf{Problem 10:} ``Does $-1$ satisfy $x\le-1$? Does it satisfy $x>-1$? So which
        rule gets it?''
  \item \textbf{Problem 12, the crux transferred:} ``You changed the second rule. Did you change
        the \emph{inequality}? Then who owns $-1$?''
  \item \textbf{Problem 13:} ``$-1.2$ --- which integer is just \emph{below} it on the number
        line?''
\end{itemize}
While circulating, pick two papers to put up in the debrief --- one problem~12 with the closed
dot at $(-1,2)$ and the open dot at $(-1,3)$, and one that moved $p(-1)$ to $3$.
...
\begin{enumerate}[leftmargin=1.3em, nosep, itemsep=3pt]
  \item \textbf{Put $C(m)$ and $P(m)$ back up side by side} with their dots, and make a student
        ...
        jumps.}
  \item Put up the two problem~12s you picked. The one that moved $p(-1)$ to $3$ read the
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{Giving a boundary input to the strict-$<$ piece, or to ``both''} (notes~2, 10
        and~12, HW~1 and~6) --- the lesson's target misconception. Send them to the
        inequality, not to the story. Probe: \emph{``Does it satisfy $x\le2$? Does it satisfy
        $x>2$? So which rule gets it?''}
  \item \textbf{Reading the open dot as the value at a breakpoint} (notes~7, the crux; 12;
        HW~3). Probe: \emph{``Two dots at that $x$. Which one is filled in?''}
  \item Calling every piecewise graph ``discontinuous'' --- pieces \emph{can} meet; check whether
        the two rules agree there (notes~3 and~5, 11, HW~2).
  \item Greatest-integer sign error: rounding \emph{toward zero}, so $\lfloor-1.2\rfloor=-1$
        (notes~9, 13, HW~4). Probe: \emph{``Which integer is just below it?''}
  \item Saying a step function is ``increasing'' on a step --- it is \textbf{constant} there,
        then jumps (the \emph{Boundary} row's staircase, notes~9).
...
hanging; the hook's month-3 vote and the \emph{Several Rules} row's boundary rule pay it off within five minutes. The
...
term is named}, never in advance, and take the month-3 vote without resolving it. Then the table,
row by row: read the definition, mark up the display, work the first problem of the grid (1, 3,
5, 7), and hand the rest to the room. \emph{Several Rules} must move: five minutes on the brace,
$C(5)$, the boundary rule, and problems 1--2. \emph{Two Pieces} is four: the brace for $|x|$, the
two \texttt{work} blocks for the trail, and problem~3. \textbf{Do not cut ``both rules give
$H(3)=1$, so they meet''} --- it is the first meets example, and \emph{Meets or Jumps} needs it.

\textbf{Spend the time in the \emph{Boundary} row}, which carries the misconception. First
\emph{Meets or Jumps}: total paid meets, price this month jumps (5--6), and the one test read off
the two pictures before naming anything. Then label the two dots at $m=3$ and let problem~7
happen: someone says ``$P(3)=12$, the price is \$12 after the free months'' and the room catches
it. Do not resolve it by repeating the rule; point at the two dots. Close the row on the
staircase --- the four floor values of problem~9 (the last cell, $\lfloor-2.5\rfloor=-3$, is the
trap), the ``down means more negative'' sentence, and $[n,n+1)$ closed left, open right, which is
the boundary rule one step at a time.

\textbf{Guided Practice (10--13) is where the pen changes hands.} 10 is the boundary check, 11
the meets test and the graph read, \textbf{12 the crux transferred} --- change the second piece to
$x+4$ and the rules stop agreeing, but the \emph{inequality} has not changed, so the first piece
still owns $-1$ --- and 13 rounding down with ``does the step own $5$?'' If time is short, cut 11,
never 12.
...
read comes from. Two \texttt{work} blocks in the notes (the two halves of the trail), byte-identical
in blank and key; every other answer space is a fixed-height grid cell.

\textbf{Debrief (10 min), spoken.} $C$ and $P$ side by side, then the two problem~12s; item~2 is
the one move that cannot be cut. \textbf{Do not let the debrief eat the last ten minutes} --- the
