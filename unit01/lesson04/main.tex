...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    They connect back to Lesson~1.3 by writing the absolute value function as the two-piece rule
    ...
    (\textbf{A2.F.2f}). Absolute value (1.3) and the greatest-integer function are the two named entry
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Absolute value is the bridge: Lesson~1.3's V is exactly a two-piece linear function.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    rule} at several inputs (Lesson~1.1) --- the mechanical skill each piece needs; \textbf{(2)} recall
    from Lesson~1.3 that the V $y=|x|$ is really \emph{two lines} ($y=x$ for $x\ge 0$, $y=-x$ for $x<0$)
...
\textbf{2. Absolute value is piecewise.} Bridge from 1.3: $|x|=\{\,x \text{ if } x\ge 0;\ -x \text{ if }
...
pieces. Connect forward: 1.5 fits a \emph{line} to data.
\end{multicols}
\end{skillbox}
...
open-vs-closed slip / continuity slip / floor-rounding slip'' to decide whether Lesson~1.5 needs a
...
graph, and reason about tax-bracket-style continuity. \textbf{Preview:} Lesson~1.5, \emph{Linear
...
Lesson~1.3: the V is literally two linear rules, $y=x$ and $y=-x$, joined at the vertex; that is the
...
boundary'' is the whole idea. Section~2 ties back to 1.3: the V is the first piecewise function they
already knew.
\end{teachernote}
...
Lesson~1.5 needs a two-minute re-teach of piece selection.
\end{teachernote}
