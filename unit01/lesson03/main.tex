...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    increasing/decreasing intervals off the V. Lesson~1.2 \emph{solved} $|ax+b|=c$; today that same
    ...
    Unit~2 formalizes for parabolas.
    \\[2pt]
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item The vertex is the graph's \emph{turning point}: where increasing meets decreasing (previewing Unit~2).
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    and never dips below it; \textbf{(2)} recall the transformation language from Lesson~1.1 ---
...
Note it is the two lines $y=x$ and $y=-x$ pasted at the vertex --- a preview of piecewise (1.4).
...
a graph, or complete a table and name the vertex. Connect back to 1.2: the solutions of $|x-1|=2$ are
...
vertex-read slip / range slip'' to decide whether Lesson~1.4 opens with a quick re-teach of vertex
form.
\end{skillbox}
...
\textbf{Preview:} Lesson~1.4, \emph{Piecewise-Defined Functions} --- today's V is really \emph{two}
...
smallest value ($0$ at $x=0$) becomes the vertex/minimum. Item~2 reactivates the Lesson~1.1 language
...
picture. Connect to Lesson~1.2: the solutions of $|x-1|=2$ are the two $x$'s where this V meets the
line $y=2$.
\end{teachernote}
...
up-vs-down slip / vertex-read slip / range slip'' to decide whether Lesson~1.4 opens with a quick
