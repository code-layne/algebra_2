%! Absolute Value Equations \& Inequalities — Unit 1, Lesson 1.2 — Experience & Formalize (Activity · QuickNotes · Application · Check Your Understanding) (Answer Key)
...
\pageheader{Unit 1, Lesson 1.2}{Experience \& Formalize: In Range --- Answer Key}
...
Six exercises --- this is the lesson's practice, and there is \textbf{no homework}. Work 1--3 with a
partner, then finish 4--6 on your own. Isolate the bars \emph{before} you split.
...
\begin{enumerate}[leftmargin=1.7em, itemsep=7pt]
  ...
  \item \textbf{``Great\emph{OR}.''} Solve $|3x - 6| \ge 9$, then write the set three ways.
    ...
    \end{work}
    Set: \ans{$\{x \mid x\le -1 \text{ or } x\ge 5\}$} \quad Interval: \ans{$(-\infty,-1]\cup[5,\infty)$} \\[4pt]
    \hspace*{1em}
    \begin{tikzpicture}[x=0.44cm, y=0.44cm, baseline=-2pt]
      \numline{-4}{8}
      \draw[very thick, keyred] (-4.5,0)--(-1,0);
      \draw[very thick, keyred] (5,0)--(8.5,0);
      \fill[keyred] (-1,0) circle (2.6pt);
      \fill[keyred] (5,0) circle (2.6pt);
    \end{tikzpicture}

  \item \textbf{The odd ones out.} Answer each from the \emph{distance} meaning --- not a memorized
        rule. \\[3pt]
        (a) $|x - 4| = -3$ has \ans{no solution} \qquad (b) $|x - 4| = 0$ has \ans{one: $x=4$}
    \answerspace{1.2cm}{$|x-4|$ is the distance from $x$ to $4$. No point sits $-3$ away --- a distance is never negative --- so (a) is impossible; exactly one point sits $0$ away, namely $4$ itself.}
        (c) For which values of $k$ does $|x - 3| < k$ have \emph{no} solution? \ans{$k \le 0$}
    \answerspace{1.0cm}{No distance is less than a negative number, and none is less than $0$ either --- at $k=0$ it would need a distance below $0$.}
