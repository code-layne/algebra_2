...
\begin{frame}[t, plain]
  ...
  \end{columns}
  \vspace{3pt}
  \begin{block}{Tolerance --- how far from a desired value are we willing to be?}
    ``Within 5 miles of the tower'': desired value $12$, tolerance $5$, so $|x-12|\le5$.
    Every ``within so much of'' sentence is $|x-\text{desired}|\le\text{tolerance}$ --- the
    ``and'' case. \; Bracket + filled dot: \textbf{in}; parenthesis + open dot: \textbf{out}.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{6pt}
    ...
    \item[\textbf{c.}] Read part~b as a tolerance: what is the desired value, and how far may
          you stray from it?
    \item[\textbf{d.}] $2|x-5| - 3 \ge 7$. Same three ways.
  \end{enumerate}
  \vspace{5pt}
...
\begin{frame}[t, plain]
  ...
  \begin{block}{Homework --- scored, due the first class after two study halls}
    ...
    odd cases and their boundary, a thermostat's tolerance, and one multiple-choice item. Whatever is left is
    due the first class after two study halls --- I will announce the date in class and on TurtleNet.
  \end{block}
  \vspace{4pt}
