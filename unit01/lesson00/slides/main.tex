...
\begin{frame}[t, plain]
  ...
  \end{itemize}
  \vspace{5pt}
  \begin{block}{Guided Practice, together: $h(x)=-2x+6$}
...
\begin{frame}[t, plain]
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{enumerate}\setlength{\itemsep}{5pt}
        \item \textbf{Sunday evening} --- falling $2^\circ$/hr from $6^\circ$: rule, slope, both
              intercepts, sign intervals --- and the $4^\circ$-and-falling question.
        \item \textbf{Side by side} --- same zero as Saturday, opposite signs. Why only at $h=3$?
  ...
  \begin{block}{The rule}
    Which way it \emph{moves}, and where it \emph{sits} --- two different questions. Stuck? Look
    \emph{up the page} at the notes, not at your neighbour.
  \end{block}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{enumerate}\setlength{\itemsep}{5pt}
    \item \textbf{1 --- Sunday labelled completely:} slope $-2$, $y$-intercept $(0,6)$, zero
          $(3,0)$, positive before 7~p.m., negative after.
    \item \textbf{The must-land moment:} $4^\circ$ and falling. \emph{Positive} = where it
          \textbf{sits}; \emph{decreasing} = which way it \textbf{moves}. Both at once.
    \item \textbf{2 --- same zero, opposite signs:} both lines cross at $h=3$, so a line changes
          sign \emph{only} at its zero, and therefore at most once.
    \item \textbf{3(d):} a \$10 wash does not move the start; it steepens the line, so the card runs
          out \emph{sooner} ($w=4$).
  \end{enumerate}
\end{frame}
...
\begin{frame}[t, plain]
  ...
  \begin{block}{Homework --- start it now}
    ...
    is left is due next class. Six problems plus an extension:
