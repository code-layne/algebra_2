...
\begin{enumerate}[label=\textbf{\alph*.}, start=3, leftmargin=1.8em, itemsep=4pt]
  \item Circle where the line crosses the \emph{vertical} ($T$) axis:
        $({\color{keyred}\mathbf{0}},\ {\color{keyred}\mathbf{-6}})$. In the story, that point tells you:
        \answerspace{1.7cm}{At 6:00 a.m.\ ($h=0$), when the story starts, it was $-6\,^\circ$C --- the starting temperature.}
  \item Circle where it crosses the \emph{horizontal} ($h$) axis:
        $({\color{keyred}\mathbf{3}},\ {\color{keyred}\mathbf{0}})$. What time of day is that, and why is that moment
        special?
        \answerspace{1.7cm}{$h=3$ is 9:00 a.m. The temperature is exactly $0\,^\circ$C --- freezing: neither above nor below.}
\end{enumerate}
\end{minipage}\hfill
...
\begin{enumerate}[label=\textbf{\alph*.}, start=5, leftmargin=1.8em, itemsep=4pt]
  \item Every hour the temperature changes by \ans{$+2$} $^\circ$C. Circle where that number
        shows up in your rule, then say how you \emph{see} it on the graph moving from one hour
        to the next.
        \answerspace{1.8cm}{It is the $2$ multiplying $h$ in $T(h)=2h-6$: every step of $1$ to the right goes up $2$ --- the same everywhere, which is why the graph is straight.}
  \item As the morning goes on, is the temperature going \textbf{up} or \textbf{down}? How does
        ...
        \answerspace{1.3cm}{Up. Read left to right, the line rises --- each point is higher than the one before it.}
        Below freezing: $T<0$ when \ans{$0\le h<3$} \qquad
        Above freezing: $T>0$ when \ans{$3<h\le 6$}
\end{enumerate}
\end{scenariobox}
