%! Unit 1: Linear Functions — Unit Test --- Version B (KEY)
...
\pageheader{Unit 1: Linear Functions}{Unit Test --- Version B --- Answer Key}
\namedateperiod
...
\begin{tikzpicture}[scale=0.34]
  \lgrid{-3}{3}{-3}{3}
  \draw[very thick, navy] (1,-3)--(1,3);
\end{tikzpicture}\\[2pt]
1.\ \slot{2.4cm}{undefined}
\end{minipage}%
\begin{minipage}[t]{0.24\textwidth}\centering
\begin{tikzpicture}[scale=0.34]
  \lgrid{-3}{3}{-3}{3}
  \draw[very thick, navy] (-3,1)--(3,-2);
\end{tikzpicture}\\[2pt]
2.\ \slot{2.4cm}{negative}
\end{minipage}%
\begin{minipage}[t]{0.24\textwidth}\centering
\begin{tikzpicture}[scale=0.34]
  \lgrid{-3}{3}{-3}{3}
...
\begin{tikzpicture}[scale=0.34]
  \lgrid{-3}{3}{-3}{3}
  \draw[very thick, navy] (-3,-3)--(3,1.5);
\end{tikzpicture}\\[2pt]
4.\ \slot{2.4cm}{positive}
\end{minipage}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=6pt, label=\arabic*., start=5]
  \item The line $y=-4$: \hfill \slot{2.4cm}{zero}
  \item The line through $(-3,\,-2)$ and $(1,\,6)$: \hfill \slot{2.4cm}{positive}
  \item The line through $(5,\,-1)$ and $(5,\,3)$: \hfill \slot{2.4cm}{undefined}
  \item The line $y=-x+2$: \hfill \slot{2.4cm}{negative}
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=10pt, label=\arabic*., start=9]
  \item The line through $(1,\,2)$ and $(3,\,8)$.
    \begin{work}
      m &= \frac{8-2}{3-1} = \frac{6}{2} = 3 \\
      2 &= 3(1) + b \quad\Rightarrow\quad b = -1
    \end{work}
    \vspace{0.5cm}
    \hfill Equation: \slot{3.6cm}{$y=3x-1$}

  \item The line through $(-3,\,4)$ and $(3,\,0)$.
    \begin{work}
      m &= \frac{0-4}{3-(-3)} = \frac{-4}{6} = -\frac{2}{3} \\
      4 &= -\frac{2}{3}(-3) + b = 2 + b \quad\Rightarrow\quad b = 2
    \end{work}
    \vspace{0.5cm}
    \hfill Equation: \slot{3.6cm}{$y=-\tfrac{2}{3}x+2$}

  \item The line with slope $4$ that passes through $(1,\,3)$.
    \begin{work}
      y-3 &= 4(x-1) \\
      y-3 &= 4x-4 \\
      y &= 4x-1
    \end{work}
    \vspace{0.5cm}
    \hfill Equation: \slot{3.6cm}{$y=4x-1$}

  \item The line with slope $-\tfrac{3}{4}$ that passes through $(4,\,2)$.
    \begin{work}
      y-2 &= -\frac{3}{4}(x-4) \\
      y-2 &= -\frac{3}{4}x+3 \\
      y &= -\frac{3}{4}x+5
    \end{work}
    \vspace{0.5cm}
    \hfill Equation: \slot{3.6cm}{$y=-\tfrac{3}{4}x+5$}
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=10pt, label=\arabic*., start=13]
  \item \textbf{Evaluate.} (8 pts) \quad
    $f(x)=\begin{cases} x+5, & x<-2 \\ 2, & -2\le x\le 3 \\ 3x-4, & x>3 \end{cases}$
    \\[8pt]
    $f(-4)=$ \slot{1.4cm}{$1$} \qquad $f(-2)=$ \slot{1.4cm}{$2$} \qquad
    $f(3)=$ \slot{1.4cm}{$2$} \qquad $f(4)=$ \slot{1.4cm}{$8$}
  ...
  \item \textbf{Domain.} (12 pts) Give the domain of each function as an inequality or in interval
    ...
    \begin{enumerate}[label=(\alph*), leftmargin=1.9em, itemsep=12pt]
      \item $g(x)=\begin{cases} 2x, & x\le 1 \\ 5, & 1<x<4 \end{cases}$
        \hfill Domain: \slot{3.8cm}{$x<4$, \ $(-\infty,\,4)$}
      \item $h(x)=\begin{cases} x-3, & -5\le x<0 \\ 2x, & 0\le x\le 6 \end{cases}$
        \hfill Domain: \slot{3.8cm}{$-5\le x\le 6$, \ $[-5,\,6]$}
      \boxguard[9]
      \item The function $k$ graphed below.
        ...
        \begin{tikzpicture}[scale=0.5]
          ...
          \fdot{-2}{-2} \hdot{2}{2}
          \draw[very thick, forest] (2,3)--(4,3);
          \fdot{2}{3} \fdot{4}{3}
        \end{tikzpicture}
        \end{minipage}%
        \hfill Domain: \slot{3.8cm}{$-2\le x\le 4$, \ $[-2,\,4]$}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=10pt, label=\arabic*., start=15]
  \item (3 pts) Circle the letter of the graph of an \textbf{absolute value} function.
    ...
    \begin{tikzpicture}[scale=0.32]
      \lgrid{-3}{3}{-3}{3}
      \draw[very thick, navy, domain=-2.4:2.4, smooth] plot (\x, {0.5*\x*\x-1.5});
    ...
    \begin{tikzpicture}[scale=0.32]
      \lgrid{-3}{3}{-3}{3}
      \draw[very thick, navy] (-3,1)--(0,-2)--(3,1);
    \end{tikzpicture}\\ \optok{D}{}
    \end{minipage}

  \item (3 pts) Which equation is an \textbf{absolute value} function?
    \\[4pt]
    \opt{A}{$y=-4x+1$} \hfill \opt{B}{$y=x^2-4$} \hfill \opt{C}{$y=\lfloor x\rfloor-2$} \hfill
    \optok{D}{$y=|x+2|-5$}
  ...
  \item (15 pts) Each function is a transformation of the parent $y=|x|$. Fill in the table.
    ...
    \begin{tabular}{|c|c|c|c|c|}
      ...
      \textbf{Opens} \\ \hline
      $g(x)=|x-1|+4$ & \slot{1.9cm}{$(1,\,4)$} & \slot{2.1cm}{right 1} & \slot{2.1cm}{up 4} &
        \slot{1.4cm}{up} \\ \hline
      $h(x)=|x+6|-3$ & \slot{1.9cm}{$(-6,\,-3)$} & \slot{2.1cm}{left 6} & \slot{2.1cm}{down 3} &
        \slot{1.4cm}{up} \\ \hline
      $k(x)=-|x-2|$ & \slot{1.9cm}{$(2,\,0)$} & \slot{2.1cm}{right 2} & \slot{2.1cm}{none} &
  ...
  \item (4 pts) Which equation matches the graph?
    ...
    \begin{tikzpicture}[scale=0.36]
      \lgrid{-3}{5}{-2}{4}
      \draw[very thick, navy] (-2.5,-1.5)--(1,2)--(4.5,-1.5);
      \fill[navy] (1,2) circle (4pt);
    \end{tikzpicture}
    \end{minipage}%
  ...
  \item (3 pts) Compared with the parent $y=|x|$, the graph of $y=4|x|$ is
    \\[4pt]
    \opt{A}{wider} \hfill \opt{B}{shifted up 4} \hfill \optok{C}{narrower} \hfill
    \opt{D}{shifted left 4}
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.9em, itemsep=10pt, label=\arabic*., start=20]
  \item (8 pts) \quad $|2x-5|=9$
    \begin{work}
      2x-5 &= 9 \quad\text{or}\quad 2x-5 = -9 \\
      2x &= 14 \quad\text{or}\quad 2x = -4
    \end{work}
    \vspace{0.5cm}
    \hfill Solution: \slot{4.2cm}{$x=7$ or $x=-2$}

  \item (8 pts) \quad $|x-4|\le 3$
    \begin{work}
      -3 &\le x-4 \le 3 \\
      1 &\le x \le 7
    \end{work}
    \vspace{0.5cm}
    \hfill Solution: \slot{4.2cm}{$1\le x\le 7$}

  \item (8 pts) \quad $|x+3|>2$
    \begin{work}
      x+3 &> 2 \quad\text{or}\quad x+3 < -2 \\
      x &> -1 \quad\text{or}\quad x < -5
    \end{work}
    \vspace{0.5cm}
    \hfill Solution: \slot{4.2cm}{$x>-1$ or $x<-5$}
\end{enumerate}
