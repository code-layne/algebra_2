...
% Vocabulary rows: copy the \vterm / \vtermans block from unit01/lesson04/notes/main.tex
% (fixed-height rows filled as each term is named).
...
% ── HOOK (stays — the 60-second context whose numbers the rows below reuse; ends on a
%    circle-one vote left unresolved) ──
\boxguard[14]
\begin{hookbox}
\small
TODO: the hook, two or three sentences, and the vote left hanging.
\par\writelines{2}
\end{hookbox}
...
\end{center}
\\
& \fcolorbox{forest}{forestbg}{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{\centering
