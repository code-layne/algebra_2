%! The Rational Parent Function \& Transformations — Unit 5, Lesson 5.4 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 5.4: The Rational Parent Function \& Transformations}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    horizontal asymptotes, from Lesson 5.0), \textbf{A2.F.2a} (domain, range, intercepts, including
    ...
    free-throw models). Builds on Units 2--3 transformations and Lesson 5.3's combining; it is the
    prerequisite for \textbf{5.5} (where the asymptotes stop being visible in the equation) and quietly
    seeds \textbf{5.7} (Homework 5's constant product is an inverse variation).
\end{tcolorbox}
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item ``Inside is backwards'' is not a new rule; it is Unit 2's rule, and it costs more here because it puts the wall on the wrong side of the axis.
  \item The horizontal asymptote is a \textbf{range} exclusion. Students restrict domains by habit and forget ranges entirely.
  \item A transformed parent \emph{never} crosses its horizontal asymptote --- stronger than 5.0's ``may cross,'' and for a reason worth stating: the numerator is a nonzero constant.
  \item $k$ \emph{is} the ratio of the leading coefficients. The degree rule from 5.0 and the general form are the same fact.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\textbf{Origin symmetry (4.0)} & $f(-x)=-f(x)$: rotating $180^\circ$ about the origin returns the graph. $\frac1x$ is an \emph{odd} function. \\
\textbf{Degree comparison (5.0)} & Equal degrees $\Rightarrow$ HA is the ratio of the leading coefficients --- which is exactly the $k$ left over after the rewrite. \\
\end{tabularx}
\end{skillbox}
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    from Units 2--3}: the vertices of $|x-3|+1$ and $(x+2)^2-5$, then the general statement in words
    ...
    $2+\frac{3}{x-1}$ into $\frac{2x+1}{x-1}$ (Lesson 5.3), then find its asymptotes the Lesson 5.0 way
...
intervals, never one, because an interval may not span a wall (5.0) --- and it has origin symmetry, so
it is odd (4.0).
...
the intercepts are \emph{computed}. Then the question that separates today from 5.0: \textbf{``does
this graph ever cross its horizontal asymptote?''} Setting $g(x)=1$ requires $\frac{2}{x-3}=0$, which is
impossible. A transformed parent \emph{never} crosses its HA --- stronger than 5.0's ``may cross,'' and
...
shifted, stretched $\frac1x$ --- and this is \emph{why} 5.0's degree rule is true in that case: the
...
\begin{itemize}[leftmargin=1.2em, nosep]
  \item \textbf{the sign of $h$} --- $\frac{1}{x-3}$ read as a shift \emph{left}; the signature error, and worse here than in Units 2--3 because it puts the wall on the wrong side of the $y$-axis;
  ...
  \item \textbf{decreasing on $(-\infty,\infty)$} --- 5.0's union rule, still in force, still costing points;
...
$\frac{2x-7}{x-4}=2+\frac{1}{x-4}$, then confirm both horizontal asymptotes against 5.0's degree rule.
...
re-teach before tomorrow, because 5.5 takes the asymptotes off the face of the equation entirely --- a
...
for 5.7; and a \textbf{free-throw model}, $P(x)=\frac{12+x}{20+x}=1-\frac{8}{x+20}$, where reaching
...
horizontal asymptote could never have pinned down $a$. \textbf{Preview:} Lesson 5.5, \emph{Graphing
