%! The Rational Parent Function \& Transformations — Unit 5, Lesson 5.4 — Guided Notes (Answer Key)
...
\ans{$x=0$ is not in the domain} . (Lesson 5.0: an interval may never span a wall.)
...
Unit 2 rule, costlier here. (2) \textbf{Reading $k$ off the numerator}. (3) \textbf{``The parent goes
...
--- 5.0's union rule, still in force.
...
$\frac{x-1}{x-3}=1+\frac{2}{x-3}$ runs that in reverse, and it is \emph{why} 5.0's degree rule holds
