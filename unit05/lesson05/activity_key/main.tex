%! Graphing Rational Functions — Unit 5, Lesson 5.5 — Group Activity (Answer Key)
...
\begin{enumerate}[label=\arabic*., leftmargin=2.1em, itemsep=4pt]
  ...
  \item Now compare with the graph below. Does your chart match it? \ans{yes} \\[3pt]
        ...
        \ansline{this numerator has a zero, so $f(x)=0$ has a solution; in 5.4 the numerator was a nonzero constant, so it never could}
\end{enumerate}
\begin{center}
...
\ans{no} horizontal asymptote. Divide anyway --- Lesson 4.3, long division:
\begin{center}
...
\begin{enumerate}[label=(\alph*), leftmargin=2.1em, itemsep=5pt]
  ...
  \item Compare with Lesson 5.4's disguise, $\frac{x-1}{x-3}=1+\frac{2}{x-3}$. What is the same about
        the two rewrites, and what is different about the leftover? \ansline{both divide and keep the remainder as a fraction; in 5.4 the quotient was a \emph{constant}, so the graph settled onto a horizontal line --- here the quotient is \emph{linear}, so it settles onto a slanted one}
\end{enumerate}
...
\begin{enumerate}[label=(\alph*), leftmargin=2.1em, itemsep=5pt]
  \item Combine into a single fraction (Lesson 5.3): \; $T(x)=$ \ans{$\dfrac{60x+300}{x(x+10)}$}
...
not in A2.F.2h and are never assessed} --- say so out loud. Part 3 reverses 5.4's dilution model: there
