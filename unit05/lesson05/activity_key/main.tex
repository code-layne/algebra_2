%! Solving Radical Equations \& Extraneous Solutions — Unit 5, Lesson 5.5 — Group Activity (Answer Key)
...
\vspace{4pt}
\textbf{Part 3 --- Solving backwards, in context.} From Lesson 5.2, a pendulum of length $L$ feet
has period
\[ T(L)=2\pi\sqrt{\tfrac{L}{32}} \ \text{ seconds.} \]
In 5.2 you were given $L$ and found $T$. Now do it the other way --- which means solving a radical
equation.
\begin{enumerate}[label=(\alph*), leftmargin=1.9em, itemsep=6pt, start=7]
  \item A clockmaker wants a pendulum with a period of exactly $2\pi$ seconds. Solve
        $2\pi=2\pi\sqrt{L/32}$ for $L$, and check your answer against 5.2's table.

        {\small \emph{Work:} divide by $2\pi$: $\sqrt{L/32}=1$. Square: $L/32=1$, so $L=32$.
        Lesson 5.2's table has $L=32 \Rightarrow T=2\pi$. \checkmark}
  ...
  \item Now find the length that gives a period of $\pi$ seconds. \; $L=$ \ans{$8$} feet
        ...
        (5.2's table: $L=8 \Rightarrow T=\pi$. \checkmark)}
        ...
        same fact 5.2 met from the other side (quadrupling $L$ only doubles $T$).}
...
\begin{itemize}[leftmargin=1.3em, nosep]
  ...
  \item \textbf{Tier E Part 3(i)} draws the same line Lesson 4.6 drew: a rejection for
