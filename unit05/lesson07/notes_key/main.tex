%! Inverse Functions — Unit 5, Lesson 5.7 — Guided Notes (Answer Key)
...
\begin{itemize}\setlength{\itemsep}{1pt}
  ...
  \item Where have you seen that mistake already this week? \ansline{Warm-Up item 3 --- you cannot
        undo ``double, then add $5$'' by halving first. And all of Lesson 5.6: order is part of the
        answer.}
\end{itemize}
...
$g\big(f(x)\big)=\sqrt{x^{2}}=$ \ans{$|x|$} \; (Lesson 5.1) --- which is \emph{not} $x$ when $x<0$ \\
\end{tabularx}
\end{center}
\vspace{-2pt}
...
\ans{squaring} undoes a square root. That is what ``inverse families'' meant back in Lesson 5.0.
...
\begin{itemize}[leftmargin=1.3em, nosep]
  ...
  \item \textbf{Undoing in the wrong order} --- Rosa's $\frac59f-32$, and $h(x)=\frac{x}{2}-6$ in
        \S2(b). This is Lesson 5.6 refusing to go away, and it is why \S2 comes \emph{before} \S3:
...
answer, in one glance, the question students first raised in Lesson 5.0: why does $\sqrt{x}$ get
