%! Simplifying Radicals; Adding \& Subtracting — Unit 5, Lesson 5.2 — Slides (Beamer)
  ...
  \begin{columns}[T]
    \begin{column}{0.44\textwidth}
      \begin{block}{A radical is simplified when}
        ...
        {\scriptsize (Condition 3 is Lesson 5.3.)}
      \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
...
\begin{frame}[t, plain]
  ...
  \begin{itemize}\setlength{\itemsep}{3pt}
    \item \textbf{5.3} The same property run \emph{forwards}: multiplying, distributing, FOIL, and
          rationalizing a denominator with a \textbf{conjugate}.
    \item \textbf{5.4} Graphing $y=a\sqrt{x-h}+k$ --- where simplifying $\sqrt{8x}$ into
          $2\sqrt{2x}$ turns out to be a \emph{vertical stretch}.
    \item \textbf{5.5} Solving radical equations, and why extraneous solutions appear.
