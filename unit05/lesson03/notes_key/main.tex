%! Adding \& Subtracting Rational Expressions — Unit 5, Lesson 5.3 — Guided Notes (Answer Key)
...
\ans{hole} in the graph; $(x+2)$ stayed, so $x=-2$ is a \ans{wall}. That sort is Lesson 5.0's,
...
Reading anything off your final answer is the error from Lesson 5.1, and it is still wrong today.

\vspace{5pt}
\textbf{Opposite binomials in the denominators} (Lesson 5.1, still in force). \;
...
one simple fraction, then keep--change--flip (Lesson 5.2).
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
...
2 & the \emph{whole bottom} & the main bar is a $\div$, and you cannot divide by zero. This is Lesson 5.2's \textbf{source 3} in a new costume: set the bottom's combined \emph{numerator} to zero. Here $4-x=0$ gives $x\neq$ \ans{4}. \\
\hline
\end{tabularx}
\end{center}
...
(4) Cancelling a term out of a sum before it is combined (Lesson 5.1's ghost). (5) Forgetting the
...
denominators are still there, and the bottom's numerator is the divisor's numerator from 5.2. Ask which
...
from denominators that no longer appear. It is the same picture 5.5 will build from an equation, and
the same fact 5.6 will call an extraneous solution.
\end{teachernote}
