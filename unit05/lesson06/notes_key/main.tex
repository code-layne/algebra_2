%! Composition of Functions — Unit 5, Lesson 5.6 — Guided Notes (Answer Key)
...
From Lesson 5.4, that is the parent shifted \ans{right $4$}. &
From Lesson 5.4, that is the parent shifted \ans{down $4$}. \\
\end{tabularx}
\end{center}
\begin{center}
...
\textbf{$(g\circ f)(x)=\sqrt{x^{2}}=$ \ans{$|x|$}} \; (Lesson 5.1!) \\
\hline
...
it makes 5.7 and next year's calculus easier.
\begin{itemize}[leftmargin=1.3em, nosep]
  ...
  \item \textbf{Section 5 is the payoff of the whole unit.} $\sqrt{x-4}$ and $\sqrt{x}-4$ are graphs
        they have already read in 5.4 --- put both on the board and ask which one the composition
        $f(g(x))$ produces \emph{before} you compute it. The green box is the answer to a question
        students have been carrying since 5.4: \emph{why do inside changes go the wrong way?} They go
        the wrong way because the inner machine alters the input before the outer one sees it.
  \item \textbf{Section 6's second example is the bridge to 5.7 --- do not cut it.}
        $\left(\sqrt{x}\,\right)^{2}$ simplifying to $x$ while refusing $x=-9$ is the single most
        surprising line in the lesson, and every student who misses it will find 5.7's restricted
  ...
  \item Watch for $\sqrt{x^{2}}=x$ (dropping the absolute value). That is 5.1's
