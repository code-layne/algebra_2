%! Solving Rational Equations — Unit 5, Lesson 5.6 — Guided Notes
...
\termblanklong{Rational equation}
\termblanklong{Clearing the denominators}
\termblanklong{Candidate vs.\ solution}
...
$x^2=4$, so the candidates are $x=\blank{0.8cm}$ and $x=\blank{0.8cm}$ --- for \textbf{both}
...
\end{center}
\vspace{-2pt}
Equation (I) has \blank{0.8cm} solution(s); equation (II) has \blank{0.8cm}. The algebra could not
...
\hline
 & \textbf{Lesson 5.3 --- an \emph{expression}} & \textbf{Lesson 5.6 --- an \emph{equation}} \\
\hline
...
What is left at the end & A fraction, with restrictions & Numbers, which must then be \blank{2.0cm} \\
\hline
\end{tabularx}
\end{center}
...
\hline
1 & \textbf{Factor \& restrict} & factor every denominator; list every \blank{2.4cm} value. \emph{Write the list at the top of your work.} \\
2 & \textbf{Multiply by the LCD} & every term, including any term with no denominator; then cancel \\
3 & \textbf{Solve} & whatever is left --- a \blank{1.6cm} or a \blank{2.0cm} equation \\
...
by dividing, so it never changes the answers. Multiplying both sides by \blank{1.2cm} cannot be
...
\blank{1.2cm}. So at those inputs --- and only at those inputs --- the multiplication is not
...
equation (I) of the Hook does not produce a \emph{false} statement. It produces \blank{4.6cm}
--- which is not a statement at all. You cannot check a number into an equation where the equation does
not exist.
\end{notesbox}
...
$x\neq\blank{0.8cm}$ and $x\neq\blank{0.8cm}$. \; LCD $=\blank{2.6cm}$
...
Cancel: \quad $\rule{2.2cm}{0.4pt}-\rule{1.6cm}{0.4pt}=\rule{2.2cm}{0.4pt}$

\vspace{4pt}
\textbf{Step 3 --- solve what is left.} Distribute, collect everything on one side, and factor.
\[ 6x-6-2x=x^2-x \qquad\Longrightarrow\qquad 0=x^2-\rule{0.7cm}{0.4pt}\,x+\rule{0.7cm}{0.4pt}
   \qquad\Longrightarrow\qquad 0=(x-\rule{0.5cm}{0.4pt})(x-\rule{0.5cm}{0.4pt}) \]
Candidates: $x=\blank{0.8cm}$ and $x=\blank{0.8cm}$.

\vspace{4pt}
\textbf{Step 4 --- check.} Restriction list: \blank{2.0cm}. \; Is either candidate on it?
\blank{1.0cm} \; So \blank{1.0cm} candidates survive. Verify both in the \emph{original}:
\begin{center}
...
\end{center}
\vspace{-2pt}
Solution set: \blank{3.0cm}
\end{notesbox}

\begin{notesbox}{5. Worked Example B --- One Survives, One Does Not}
\[ \frac{x}{x+1}+\frac{2}{x-1}=\frac{2}{x^2-1} \]

\textbf{Step 1.} Factor the last denominator first: $x^2-1=(\rule{1.3cm}{0.4pt})(\rule{1.3cm}{0.4pt})$.
\; Restrictions: $x\neq\blank{0.8cm}$, $x\neq\blank{0.8cm}$. \; LCD $=\blank{3.0cm}$

\vspace{4pt}
\textbf{Step 2.} Multiply all three terms by the LCD and cancel:
\[ x(\rule{1.1cm}{0.4pt})+2(\rule{1.1cm}{0.4pt})=\rule{1.1cm}{0.4pt} \]

\vspace{2pt}
\textbf{Step 3.} $x^2-x+2x+2=2 \;\Longrightarrow\; x^2+x=0 \;\Longrightarrow\;
x(\rule{1.3cm}{0.4pt})=0$. \; Candidates: $x=\blank{0.8cm}$, $x=\blank{0.8cm}$.

\vspace{4pt}
\textbf{Step 4.} One candidate is sitting on the restriction list: $x=\blank{0.9cm}$ is
\blank{2.6cm}. Throw it away --- and notice you did \emph{not} have to substitute anything to know
that. The other candidate, $x=\blank{0.9cm}$, is checked in the original: left side
$=\dfrac{0}{1}+\dfrac{2}{-1}=\blank{1.0cm}$, right side $=\dfrac{2}{-1}=\blank{1.0cm}$. \;
Solution set: \blank{2.4cm}

\vspace{5pt}
\textbf{Now see it (A2.EI.4b, graphically).} Move everything to one side and combine with 5.3:
\[ \frac{x}{x+1}+\frac{2}{x-1}-\frac{2}{x^2-1}
   =\frac{x(x-1)+2(x+1)-2}{(x-1)(x+1)}=\frac{x^2+x}{(x-1)(x+1)}
   =\frac{x(x+1)}{(x-1)(x+1)}=\frac{x}{x-1},\quad x\neq-1 \]
Solving the equation is the same as asking where that combined function equals \blank{0.8cm}. Run
Lesson 5.5 on it: the factor $x+1$ cancels, so $x=-1$ is a \blank{1.2cm}; the factor $x-1$ survives,
so $x=1$ is a \blank{2.6cm}; and the simplified numerator is zero at $x=\blank{0.8cm}$.
...
\small
The solid dot at \blank{1.4cm} is where the combined function is zero. That is the \emph{real}
solution.
...
\vspace{4pt}
\textbf{Fill in the rule:} a true solution shows up as an \blank{2.2cm}; an extraneous candidate
shows up as a \blank{1.4cm} (or a wall).
\end{minipage}
\end{center}
\end{notesbox}
...
Restrictions: $x\neq\blank{0.8cm}$, $x\neq\blank{0.8cm}$. \; Cross-multiply:
$2(\rule{1.2cm}{0.4pt})=x(\rule{1.2cm}{0.4pt})$, so $2x+4=x^2-x$ and
$0=(x-\rule{0.5cm}{0.4pt})(x+\rule{0.5cm}{0.4pt})$. \\[3pt]
Candidates $x=\blank{0.9cm}$ and $x=\blank{0.9cm}$. On the restriction list? \blank{1.0cm} \;
Solution set: \blank{2.6cm}

\vspace{6pt}
\textbf{And sometimes everything gets thrown away.}
\[ \frac{1}{x-2}+\frac{1}{x+2}=\frac{4}{x^2-4} \]
Restrictions: $x\neq\blank{2.0cm}$. \; LCD $=\blank{2.6cm}$. \; Clearing gives
$(x+2)+(x-2)=4$, so $2x=4$ and the only candidate is $x=\blank{0.8cm}$.

\vspace{2pt}
That candidate \emph{is} on the restriction list. Every candidate is extraneous, so the equation has
\blank{2.6cm}. Write that as your answer --- it is a complete answer, not a failure to finish.
\end{notesbox}

\begin{practicebox}
Solve, showing all four steps. \; $\dfrac{6}{x^2-9}+\dfrac{1}{x+3}=1$
\begin{enumerate}[leftmargin=1.7em, itemsep=6pt]
  \item Factored denominators \blank{3.2cm}; \; restrictions $x\neq\blank{2.2cm}$; \;
        LCD $=\blank{3.0cm}$
  \item After multiplying every term by the LCD (do not forget the $1$):
        \; $\rule{1.4cm}{0.4pt}+\rule{2.0cm}{0.4pt}=\rule{2.6cm}{0.4pt}$
  \item Solve: \; $0=x^2-\rule{0.6cm}{0.4pt}\,x-\rule{0.8cm}{0.4pt}
        =(x-\rule{0.5cm}{0.4pt})(x+\rule{0.5cm}{0.4pt})$. \; Candidates \blank{2.4cm}
  \item Check. Extraneous: \blank{1.4cm} \; Solution set: \blank{2.0cm}
