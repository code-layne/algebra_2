%! Solving Rational Equations — Unit 5, Lesson 5.6 — Lesson Plan
...
\newcommand{\LessonNumberName}{Lesson 5.6: Solving Rational Equations}
...
\begin{tcolorbox}[colback=forestbg, colframe=forest, boxrule=0.9pt, arc=2mm,
    ...
    rational expression to model a contextual situation. Applied: \textbf{A2.F.2h} and the 5.5 cancel
    test (the graphical check), \textbf{A2.EO.1a/b} (5.1--5.3 restrictions and the LCD). Builds on
    \textbf{A2.EO.3b} (4.2 factoring) and Unit 3's quadratic solving. Prerequisite for \textbf{6.4},
...
\begin{itemize}[leftmargin=1.1em, nosep, after=\vspace{2pt}]
  ...
  \item Rejecting for \emph{context} is not the same as rejecting for \emph{extraneousness}; 5.7 leans on the difference.
\end{itemize}
\end{tabularx}
\end{skillbox}
...
\textbf{Restriction list} & The excluded values, taken off the \emph{original} denominators (5.1). Written first, used last. \\
\textbf{Clearing the denominators} & Multiplying \emph{every term} by the LCD so all denominators cancel. Not the same as 5.3's combining. \\
...
\begin{fixedskillbox}[Activate Prior Knowledge \& Spiral Review]{forestbg}
    ...
    with no denominator is the one students drop. Say out loud that 5.3 used the LCD to \emph{build} a
...
\begin{multicols}{2}
\textbf{1. Expression or equation?} Two minutes, and worth every one. Students just spent 5.3 building
...
substituting anything. Then the graphical check, which is where 5.5 pays its debt: combine everything
...
\begin{itemize}[leftmargin=1.2em, nosep]
  ...
  \item \textbf{restrictions read off the \emph{simplified} form} --- 5.1's signature error, which today stops being a mislabeled feature and becomes a wrong answer;
  ...
  \item \textbf{a lost distributed minus sign} when a subtracted fraction is cleared --- 5.3's lost parenthesis, still in force;
...
before 5.7}, where rejecting a solution \emph{for context} becomes the correct move and the two reasons
must stay separate.
\end{skillbox}
...
restriction $c\neq4$ is a 5.5 vertical asymptote that means the paddler exactly matches the river.
...
of $k$ --- the Hook, generalized. \textbf{Preview:} Lesson 5.7, \emph{Direct, Inverse \& Joint
