%! Simplifying Rational Expressions — Unit 5, Lesson 5.1 — Guided Notes (Answer Key)
...
\vspace{3pt}
\textbf{The procedure from Lesson 5.0, unchanged:} set the \ans{denominator} equal to \ans{0},
...
\begin{enumerate}[leftmargin=1.9em, itemsep=2pt]
  \item \textbf{Factor} the numerator and the denominator \emph{completely} (GCF first --- the Unit 4 toolkit).
...
what the question was about. In Lesson~5.0 you read that single missing point off a graph and called
it a \ans{hole}.
...
solutions in 5.6, so name the connection now. (3) Cancelling $x-5$ against $5-x$ as if they matched,
